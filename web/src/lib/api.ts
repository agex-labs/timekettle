// Thin API client. Every mutating request carries X-Timekettle-Actor (decision 8:
// auth-light but audit-ready); the actor name persists in localStorage.

// The serialized `CandidateSpec` enum (crates/timekettle-orchestrator/src/lib.rs:28).
...
// (`RunParams`, crates/timekettle-orchestrator/src/lib.rs). Values are RESOLVED: the
...
export type SessionManifest = {
  ...
  code: { sha: string | null; timekettle_version: string | null }[];
};
...
export function actor(): string {
  return localStorage.getItem("timekettle-actor") || "";
}

export function setActor(name: string) {
  localStorage.setItem("timekettle-actor", name);
}
...
export const api = {
  ...
  killRun: (id: string) => {
    ...
    return request<KillReport>(`/api/v1/runs/${id}/kill`, {
      method: "POST",
      headers: { "X-Timekettle-Actor": who },
    });
  },
};

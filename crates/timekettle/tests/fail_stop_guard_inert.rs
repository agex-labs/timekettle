...
//! every timekettle-off production request would run under different panic semantics
...
fn guard_is_a_pure_passthrough_when_replay_is_not_active() {
    ...
    let escaped = std::panic::catch_unwind(|| {
        timekettle::catch_fail_stop(|| -> u64 {
            panic!("{} synthetic, outside replay", timekettle::FAIL_STOP_SENTINEL)
        })
    });
    ...
    assert!(
        ...
        "outside replay the guard must NOT catch a panic — record-mode and \
         timekettle-off builds keep their exact panic semantics"
    );
}

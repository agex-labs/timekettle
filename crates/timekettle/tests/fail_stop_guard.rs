...
//! This file proves the stop's SCOPE: wrapped in `timekettle::catch_fail_stop` /
//! `timekettle::catch_fail_stop_async`, the panic does not unwind PAST the guard, so a
...
//! 3. a panic that is NOT a timekettle fail-stop is re-raised, not swallowed — a
...
fn fail_stop_is_contained_at_the_guard_and_never_runs_the_boundary() {
    ...
    let sync_result = timekettle::catch_fail_stop(|| {
    ...
    let async_result = block_on(timekettle::catch_fail_stop_async(guarded_probe_post("any-key")));

    // 3 — a panic that is not a fail-stop must pass straight through the guard.
    let foreign = std::panic::catch_unwind(|| {
        timekettle::catch_fail_stop(|| -> u64 { panic!("candidate bug, not a timekettle fail-stop") })
    });
    ...
    assert!(
        sync_stop.message().starts_with(timekettle::FAIL_STOP_SENTINEL),
    ...
    let payload = foreign.expect_err(
        "the guard must re-raise a panic that is NOT a timekettle fail-stop — swallowing \

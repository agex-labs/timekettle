...
//!
//! timekettle exists to catch *cross-version* regressions: record a baseline (V1),
...
//! binary. The `#[timekettle::boundary]` macro emits each function's REAL
...
//! SAME public primitives the production renderer (`timekettle-orchestrator`) and the
...
use timekettle::{addresses_for, canonical_args_hash, BoundaryEvent, KeyStamper, LookupKey};
use serde_json::json;
...
/// `#[timekettle::boundary]` record calls below are captured. Each site's explicit
...
fn recording_enabled() -> timekettle_context::ContextGuard {
    timekettle_context::enter(
        timekettle_context::ContextSnapshot::new("req-v2-regression").with_recording_decision(true),
    )
}
...
fn v2_benign_edits_resolve_while_a_real_change_diverges() {
    ...
    let events = timekettle_runtime::read_events(artifacts.path()).expect("read events");
    ...
    changed_declaration.declaration = Some(
        timekettle::BoundaryDeclaration::default()
            .effect(timekettle::EffectKind::Redis)
            .operation(timekettle::OperationKind::IdempotentDelete)
            .returns(timekettle::ReturnSemantics::Bool),
    );

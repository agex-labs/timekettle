...
//!
//! The boundary here is hand-built through the same `timekettle::__private` seam the
//! real `external_services::superposition::timekettle_boundary::read` uses (rank-2
...
/// Mirrors the vendor Superposition `read` boundary: engage only when a timekettle
...
async fn read_config(operation: &'static str, key: &str) -> Result<u64, String> {
    ...
    // Passthrough when timekettle is inactive — no observation, no allocation.
    if !timekettle::__private::observation_is_active() {
        return Ok(REAL_BODY);
    }

    let correlation = timekettle::current_correlation_id();
    let scope = format!("graceful::{operation}");
    let identity = timekettle::__private::CallsiteIdentity {
        version: 1,
        ...
        syntax_hash: Some(timekettle::__private::stable_callsite_hash(&scope)),
        span_path: timekettle::__private::current_span_path(),
    };

    let semantics = timekettle::__private::BoundarySemantics {
        replay_strategy: timekettle::ReplayStrategy::Substitute,
        kind: Some(BOUNDARY.to_string()),
        declaration: Some(
            timekettle::BoundaryDeclaration::default().operation(timekettle::OperationKind::ExternalCall),
        ),
    };
    let spec =
        timekettle::__private::BoundarySpec::with_semantics(BOUNDARY, COMPONENT, operation, semantics);
    let observation =
        timekettle::__private::CrossingObservation::with_correlation(spec, identity, caller, correlation);

    let args = json!({ "key": key });
    timekettle::__private::dispatch_async_or_miss(
        observation,
        move || args,
        ...
        |v: serde_json::Value| match v.get("Ok").and_then(serde_json::Value::as_u64) {
            Some(n) => timekettle::__private::Reconstructed::Value(Ok(n)),
            None => timekettle::__private::Reconstructed::Failed(String::from(
...
/// shared `Cargo.lock` churn) into the `timekettle` facade crate just for this test.
...
fn substitute_miss_returns_on_miss_value_in_replay() {
    ...
    let table = timekettle::LookupTable {
    ...
    let hook = timekettle::LookupTableHook::from_source(
        timekettle::LocalFileLookupSource::new(path),
        timekettle::InMemoryObservedSink::new(),
    )
    .expect("hook");
    // Install as a REPLAY hook (`is_replay() == true`).
    timekettle::set_global_runtime_hook(Some(timekettle::RuntimeHook::LookupReplay(hook)))

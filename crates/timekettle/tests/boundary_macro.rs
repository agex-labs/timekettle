...
fn recording_enabled() -> timekettle_context::ContextGuard {
    timekettle_context::enter(
        timekettle_context::ContextSnapshot::new("req-boundary-macro-recording")
...
#[timekettle::boundary(
    ...
    result = {
        (
            json!({ "output": *__timekettle_result }),
            false,
        )
    },
)]
...
#[timekettle::boundary(
    ...
    result = {
        (
            json!({ "output": __timekettle_result.as_ref().copied().ok() }),
            __timekettle_result.is_err(),
        )
    },
)]
...
#[timekettle::boundary(correlation = Some("req-boundary-default".to_string()))]
fn boundary_default(value: u64) -> u64 {
    value * 2
}

#[timekettle::redis(correlation = Some("req-redis-profile".to_string()), skip_all)]
fn redis_profile_get(key: &str) -> Result<&str, &'static str> {
    let _ = key;
    Ok("value")
}

#[timekettle::http(incoming, correlation = Some("req-http-profile".to_string()), skip_all)]
...
#[timekettle::boundary(
    ...
    args = json!({ "x": x }),
    result = { (json!({ "output": *__timekettle_result }), false) },
)]
...
impl timekettle::codec::ReplayCodec for TagCodec {
    type Value = u64;
...
fn boundary_macro_records_sync_function() {
    ...
    timekettle_runtime::set_global_runtime_hook(Some(timekettle_runtime::RuntimeHook::Recording(
        std::sync::Arc::new(
            timekettle_runtime::RecordingHook::new(artifacts.path()).expect("recording hook"),
        ),
    )))
    ...
    timekettle_runtime::flush_global_hook().expect("flush events");
    let events = timekettle_runtime::read_events(artifacts.path()).expect("events");
    ...
    // DEFAULTS: an undeclared `#[timekettle::boundary]` event keeps the default
    // `Substitute` strategy with no `kind`. The `#[timekettle::redis]` KIT (G1), by
    ...
    assert!(
        events[0].replay_strategy == timekettle::ReplayStrategy::Substitute && events[0].kind.is_none(),
        "an undeclared unit boundary uses Substitute with no kind"
    );
    assert!(
        events[7].replay_strategy == timekettle::ReplayStrategy::Execute
            && events[7].kind.as_deref() == Some("redis"),
        "the #[timekettle::redis] kit declares Execute + kind = \"redis\""
    );

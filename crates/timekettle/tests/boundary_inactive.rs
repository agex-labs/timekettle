//! Inactive-hook fast-path proof for the `#[timekettle::boundary]` family.
//!
//! This test deliberately NEVER sets `TIMEKETTLE_MODE` / `TIMEKETTLE_ARTIFACT_DIR`, so the
...
#[timekettle::boundary(
    ...
    result = { (serde_json::json!({ "output": *__timekettle_result }), false) },
)]
...
fn inactive_boundary_runs_block_without_serializing_args() {
    // No TIMEKETTLE_* env set: Déjà is inactive for this whole binary.
    assert_eq!(
        sync_probe(41),
        42,

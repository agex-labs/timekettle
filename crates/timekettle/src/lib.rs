...
//! (`timekettle::redis`, `timekettle::id`, `timekettle::time`, `timekettle::http`, `timekettle::boundary`,
//! the `#[timekettle::recordable]` trait decorator, or framework-specific helper
...
//! [`RuntimeHook`];
//! on replay (`TIMEKETTLE_MODE=replay`), the recorded result is substituted in
//! place of the live call and an `ObservedCall` is emitted for divergence
//! scoring.
//!
//! This crate is the facade: it re-exports the macros (`timekettle-derive`), the
//! recording/replay runtime (`timekettle-record`), correlation context helpers
//! (`timekettle-context`), and provides the payload-normalization helpers macros
//! expand against ([`value`], [`http`], [`db`]). Generated code reaches the
//! runtime through [`__private`].

pub use timekettle_derive::recordable;
pub use timekettle_derive::{boundary, http, id, instrument, redis, time};
...
pub use timekettle_runtime::replay::{
...
pub use timekettle_runtime::replay::{boundary_execute_mode_for, replay_strategy_to_execute_mode};
pub use timekettle_runtime::replay::{
...
/// Whether observation is off entirely (feature on, timekettle idle). Lets a caller
/// skip boot-time work that only recording or replay needs.
pub fn runtime_mode_is_disabled() -> bool {
    timekettle_runtime::runtime_mode().is_disabled()
}
...
/// `request_id` span field into timekettle-context so spawned-task boundary events
/// inherit the request correlation.
pub use timekettle_runtime::TimekettleCorrelationLayer;
/// Convenience re-export for the hook trait (needed by generated delegation).
pub use timekettle_runtime::TimekettleHook;
/// Re-export the execution graph tracing layer for framework logger setup.
pub use timekettle_runtime::ExecutionGraphLayer;
...
pub use timekettle_runtime::{catch_fail_stop, catch_fail_stop_async, FailStop, FAIL_STOP_SENTINEL};
/// Re-export semantic recording primitives so downstream crates only need
/// one `timekettle` dependency.
pub use timekettle_runtime::{
    flush_global_hook, fork_span, global_hook_from_env, installed_runtime_hook, spawn_fork,
    AsyncRecordWriter, BoundaryEvent, CompositeSink, TimekettleRecord, DisabledHook, EventBuilder,
...
/// `TIMEKETTLE_MODE=record|replay` foundation.
pub use timekettle_runtime::{
...
/// Re-export replay primitives so `timekettle::*` consumers get the full replay API.
pub use timekettle_runtime::{
...
/// (`async_bb8_diesel::ConnectionManager<timekettle::TimekettleLoadConnection<PgConnection>>`)
...
pub use timekettle_diesel::TimekettleLoadConnection;

/// The timekettle library version, for sinks that stamp provenance on the wire
/// (the recording envelope's `code.timekettle_version`).
...
pub fn replay_key_namespace() -> Option<String> {
    if replay_is_active() {
        timekettle_context::current_correlation_id()
    } else {
        None
    }
}
...
/// `timekettle_` + a sanitized prefix + a 16-hex FNV-1a suffix of the full id, so it is
...
pub fn db_schema_for(correlation: &str) -> String {
    let mut name = String::from("timekettle_");
...
/// 3. else          → `{"timekettle_opaque_type": "…"}` (type name only).
///
...
macro_rules! capture {
    ($value:expr) => {{
        ...
        (&&$crate::value::Capture(&$value)).timekettle_capture()
    }};
}
...
pub mod value {
    ...
    impl<T: ?Sized> CaptureOpaque for Capture<'_, T> {
        fn timekettle_capture(self) -> serde_json::Value {
            serde_json::json!({
                "timekettle_opaque_type": std::any::type_name::<T>(),
            })
        }
    }
    ...
    /// a call to this for boundaries opted into replay (`#[timekettle::…(replay)]`).
    ...
    /// `Err` is recorded as a non-reconstructable sentinel (`{"timekettle_err": …}`)
    ...
    pub fn result_serialize_ok<T: serde::Serialize, E: std::fmt::Debug>(
        ...
        match result {
            ...
            Err(error) => (
                serde_json::json!({ "timekettle_err": format!("{error:?}") }),
                true,
            ),
        }
    }
    ...
    /// Debug-string sentinel, `{"timekettle_err": …}`), this captures the error in a
    ...
    #[serde(tag = "result")]
    pub enum TimekettleDatabaseResultPayload {
        Ok {
    ...
    /// [`TimekettleDatabaseResultPayload`], which the DB path already shares.
    ///
    /// # Why this exists
    ///
    ...
    /// A versioned envelope around [`TimekettleDatabaseResultPayload`].
    ///
    ...
    #[derive(Debug, Clone, serde::Serialize, serde::Deserialize, PartialEq)]
    pub struct TimekettleDatabaseResult {
        pub version: u8,
        #[serde(flatten)]
        pub payload: TimekettleDatabaseResultPayload,
    }

    impl TimekettleDatabaseResult {
        ...
        pub fn ok(value: serde_json::Value, type_name: impl Into<String>) -> Self {
            Self {
                ...
                payload: TimekettleDatabaseResultPayload::Ok {
                    value,
    ...
    ///
    /// Emits the [`TimekettleDatabaseResult`] shape: an `Ok` records the value via
        ...
        T: serde::Serialize,
    {
        let record = match result {
            Ok(value) => TimekettleDatabaseResult::ok(
                serde_json::to_value(value).unwrap_or(serde_json::Value::Null),
                std::any::type_name::<T>(),
            ),
            Err(error) => {
                let (kind, message) = extract_kind(error);
                TimekettleDatabaseResult::err(kind, message)
            }
        };
...
pub mod codec {
    ...
    /// `replay_codec = ::timekettle::codec::SerdeCodec<MyType>` for a custom site.
    ...
    ///
    /// The wire envelope is byte-compatible with [`crate::value::TimekettleDatabaseResult`]
        ...
        E: serde::Serialize + serde::de::DeserializeOwned + error_stack::Context,
    {
        ...
        fn capture(value: &Self::Value) -> (serde_json::Value, bool) {
            match value {
                Ok(inner) => (
                    serde_json::json!({
                        "version": crate::value::TimekettleDatabaseResult::VERSION,
                        "result": "Ok",
    ...
    /// (`timekettle::db::get_result_captured` and friends). The tape shape is the
    /// inner codec's, unchanged: the wire rows travel on the event's row image
    /// (attached by `timekettle::db::recorded_output_with_wire`), never in the
...
pub mod db {
    ...
    /// wire bytes + type OID verbatim. Produced by the `timekettle-diesel` wrapper,
    /// carried to the boundary IN-BAND as the second half of the pair the
    /// captured query helpers return.
    pub use timekettle_runtime::wire_capture::{WireColumn, WireRow};
    ...
    pub fn query_state_key(
        ...
        timekettle_runtime::replay::db_query_state_key(operation, table, sql, inputs).to_wire()
    }
    ...
    pub fn table_from_event_args(args: &serde_json::Value) -> Option<&str> {
        timekettle_runtime::replay::db_table_from_event_args(args)
    }
    ...
    /// `timekettle-diesel` connection wrapper before `FromSql` consumed them.
    ...
    impl DbRowImage {
        ...
        pub fn new(table: impl Into<String>, columns: Vec<DbColumnImage>) -> Self {
            Self {
                timekettle_image: Self::KIND.to_string(),
                version: Self::VERSION,
                table: table.into(),
                columns,
                wire_format: None,
    ...
    pub fn row_state_key(table: &str, row: &serde_json::Value) -> Option<crate::StateKey> {
        timekettle_runtime::replay::db_row_state_key(table, row)
    }
    ...
    pub fn row_state_keys(table: &str, value: &serde_json::Value) -> Vec<crate::StateKey> {
        timekettle_runtime::replay::db_row_state_keys(table, value)
    }
    ...
    pub fn binds_read_keys(table: &str, sql: &str) -> Vec<String> {
        let Some(identity) = timekettle_runtime::replay::table_identity_columns(table) else {
            return Vec::new();
        };
        ...
        for index in 0..leading.len() {
            let row: serde_json::Map<String, serde_json::Value> = per_column
                ...
                .collect();
            if let Some(key) =
                timekettle_runtime::replay::db_row_state_key(table, &serde_json::Value::Object(row))
            {
    ...
    /// timekettle-side change needing no vendor edit.
    ///
...
/// Not part of the public API — the `timekettle::*` attribute macros call these.
pub mod __private {
    pub use timekettle_context::current_correlation_id;
    // The single boundary-crossing seam the `#[timekettle::boundary]` family emits.
    ...
    // backward compatibility (they are `#[deprecated]` in timekettle-record and are
...
mod capture_tests {
    ...
    fn runtime_serialize_failure_records_tagged_marker_not_null() {
        ...
        assert!(
            captured.get("timekettle_unserializable").is_some(),
...
mod db_row_image_tests {
    ...
    fn row_images_with_metadata_attach_matching_columns_only() {
        ...
        let image = &images[0];
        assert_eq!(image.timekettle_image, "db_row");
...
#[cfg(test)]
mod db_result_tests {
    use crate::value::{result_serialize_db, TimekettleDatabaseResult, TimekettleDatabaseResultPayload};

    /// `TimekettleDatabaseResult` round-trips through serde for the Ok variant,
    /// preserving the (possibly large-integer) value and its type name.
    #[test]
    fn ok_round_trips_through_serde() {
        let original = TimekettleDatabaseResult::ok(serde_json::json!(42), "usize");
        ...
        let decoded: TimekettleDatabaseResult = serde_json::from_value(encoded).expect("decode");
        ...
        let decoded: TimekettleDatabaseResult =
            serde_json::from_value(stringified).expect("decode stringified big int");
        match decoded.payload {
            TimekettleDatabaseResultPayload::Ok { value, type_name } => {
    ...
    /// `TimekettleDatabaseResult` round-trips through serde for each Err kind.
    #[test]
    fn err_round_trips_for_each_kind() {
        for kind in ["NotFound", "UniqueViolation", "Other"] {
            let original = TimekettleDatabaseResult::err(kind, format!("{kind} message"));
            let encoded = serde_json::to_value(&original).expect("encode");
            assert_eq!(
                encoded,
                serde_json::json!({
    ...
    // Stand-in for `errors::DatabaseError` (the timekettle crate does not depend on
    ...
    fn record_then_recover_round_trip() {
        ...
        match decoded.payload {
            TimekettleDatabaseResultPayload::Err { kind, message } => {
                assert_eq!(

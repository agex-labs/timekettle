...
use timekettle_orchestrator::scope::{RunScope, ScopedRecording, TapeSlot};
use timekettle_orchestrator::{
...
fn write_run(root: &HarnessRoot, run_id: &str, recording_id: &str, filter: Option<Vec<String>>) {
    ...
    timekettle_orchestrator::write_json(&root.run_path(run_id), &run).expect("write run");
}
...
fn every_artifact_a_scoped_run_produces_stays_inside_its_scope() {
    ...
    let run: Run = timekettle_orchestrator::read_json(&root.run_path(run_id)).expect("read run");
    ...
    timekettle_orchestrator::write_json(&root.lookup_table_path(run_id), &table).expect("write table");
    ...
    let anchors: HashSet<u64> = recording
        ...
        .filter_map(|item| match item {
            timekettle_orchestrator::scope::TapeItem::Event(e) => e.graph_node_id,
            timekettle_orchestrator::scope::TapeItem::Malformed { .. } => None,
        })
        .collect();

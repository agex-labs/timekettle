...
/// `TIMEKETTLE_S3_BUCKET` (e.g. a shared codebundle bucket).
...
pub fn stage_bundle(uri: &str, dest: &Path) -> Result<usize, String> {
    ...
    cfg.bucket = bucket;
    let bytes = timekettle_compactor::get_object_decoded(&cfg, &key)?;
    if bytes.is_empty() {
...
pub const CANDIDATE_CONFIG_FILES_ENV: &str = "TIMEKETTLE_CANDIDATE_CONFIG_FILES";
...
/// `TIMEKETTLE_<SYSTEM>_CANDIDATE_CONFIG_FILES` for a non-default system and fall
...
pub fn ensure_bundle_staged(
    ...
    if timekettle_compactor::object_exists(cfg, &key)? {
    ...
    timekettle_compactor::put_object(cfg, &key, tar)?;
...
/// `TIMEKETTLE_HTTP_PROXY` first — a DEDICATED var so it scopes to this one outbound
...
mod tests {
    ...
    fn git_manifest_and_tar_from_a_real_repo() {
        ...
        let _ = git(&["config", "user.name", "timekettle-test"]);
    ...
    // TIMEKETTLE_NET_TESTS=1 (keeps the suite offline/deterministic by default).
    #[test]
    fn targz_producer_against_real_codeload() {
        if std::env::var("TIMEKETTLE_NET_TESTS").ok().as_deref() != Some("1") {
            eprintln!("skipping: set TIMEKETTLE_NET_TESTS=1 to run the codeload fetch");
            return;
        }
    ...
    fn extracts_all_real_hyperswitch_migrations() {
        let dir = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../vendor/hyperswitch-timekettle-clean/migrations");
        if !dir.exists() {
            eprintln!(

...
//! see `timekettle-compactor`). Both ingest paths here READ; neither writes. A replay
...
pub use timekettle_compactor::S3Config;
...
pub use timekettle_compactor::{correlation_count, read_correlation_index, CorrelationSummary};
...
pub struct IngestReport {
    ...
    /// balance needs — a marker is not a `TimekettleRecord` and never joins the event
...
impl MarkerLedger {
    fn record(&mut self, line: &str) {
        ...
        let Some(body) = probe.marker else {
            // A `timekettle_sink_marker` envelope with no `marker` body claims
...
/// The wire `artifact_type` values, and the `TimekettleRecord` tag each becomes.
///
...
const ARTIFACT_TYPE_MARKER: &str = "timekettle_sink_marker";
const ARTIFACT_TYPE_GRAPH_NODE: &str = "timekettle_graph_node";
...
impl<'a> EnvelopeProbe<'a> {
    ...
    fn payload(&self) -> Option<(&'static str, &'a serde_json::value::RawValue)> {
        match self.artifact_type.as_deref() {
            ...
            // `timekettle_artifact_record`, and an unset type for legacy envelopes.
...
pub fn count_session_objects(cfg: &S3Config, recording_id: &str) -> Result<usize, String> {
    timekettle_compactor::count_landing_objects(cfg, recording_id)
}
...
pub fn pull_recording(
    ...
    recording_id: &str,
    dest: &Path,
) -> Result<(IngestReport, timekettle_compactor::SessionManifest), String> {
    let manifest = match timekettle_compactor::read_manifest(cfg, recording_id)? {
        Some(m) => m,
        None => timekettle_compactor::compact_session(cfg, recording_id)?,
    };
    let lines = timekettle_compactor::read_session_lines(cfg, &manifest)?;
    ...
    let report = IngestReport {
        prefix: timekettle_compactor::layout::session_root(recording_id),
...
/// whose lines are full `timekettle.artifact_record/v2` envelopes with sessions
...
pub fn pull_recording_from_prefix(
    ...
    let keys = timekettle_compactor::list_objects(cfg, prefix)?;
    if keys.is_empty() {
        return Err(format!(
    ...
    for key in &keys {
        let data = timekettle_compactor::get_object_decoded(cfg, key)?;
...
/// event object, so the unwrapped line deserializes as a `TimekettleRecord`. Preserves
        ...
        for line in chunk.split(|&b| b == b'\n') {
            ...
            let (record_kind, event_raw): (&'static str, String) =
                match serde_json::from_str::<EnvelopeProbe>(&line_str) {
                    ...
                    // The canonical events.jsonl is a `TimekettleRecord` stream, internally
                    ...
                    // can deserialize the line as a `TimekettleRecord`.
...
mod tests {
    ...
    fn envelope(rid: &str, gseq: u64, payload_extra: &str) -> String {
        format!(
            r#"{{"schema_version":2,"artifact_type":"timekettle_artifact_record","instance_id":"router-h-1","event":{{"recording_run_id":"{rid}","global_sequence":{gseq}{payload_extra}}}}}"#
        )
    }
    ...
    fn at_boundary(
        ...
        format!(
            r#"{{"schema_version":2,"artifact_type":"timekettle_artifact_record","instance_id":"{instance}","event":{{"recording_run_id":"r1","global_sequence":{gseq},"correlation_id":"{correlation}","request_sequence":{rseq},"boundary":"{boundary}"}}}}"#
        )
    }
    ...
    fn marker_envelope(instance: &str, body: &str) -> String {
        format!(
            r#"{{"schema_version":2,"artifact_type":"timekettle_sink_marker","instance_id":"{instance}","recording_run_id":"r1","capture":{{"mode":"session","session_id":"r1"}},"marker":{body}}}"#
        )
    }
    ...
    /// both copied from the producer (`GraphEnvelope` in the router's timekettle
    ...
    fn graph_envelope(rid: &str, gseq: u64) -> String {
        format!(
            r#"{{"schema_version":1,"artifact_type":"timekettle_graph_node","instance_id":"router-h-1","recording_run_id":"{rid}","capture":{{"mode":"session","session_id":"{rid}"}},"node":{{"recording_run_id":"{rid}","global_sequence":{gseq},"node_id":{gseq},"span_name":"payments_create"}}}}"#
        )
    }
    ...
    fn every_line_read_is_accounted_for() {
        ...
        let chunk = format!(
            ...
            eof("router-h-1", 1),
            r#"{"artifact_type":"timekettle_artifact_record","instance_id":"h"}"#, // no payload
        )
        .into_bytes();
        let Collated {
            events,
    ...
    fn a_graph_node_does_not_displace_the_boundary_event_of_the_same_sequence() {
        ...
        // deserializable as the right `TimekettleRecord` variant.
    ...
    fn a_marker_whose_body_cannot_be_read_is_named_not_assumed_fine() {
        ...
        let chunk = format!(
            "{}\n{}\n{}\n",
            r#"{"artifact_type":"timekettle_sink_marker","instance_id":"h","recording_run_id":"r1"}"#,

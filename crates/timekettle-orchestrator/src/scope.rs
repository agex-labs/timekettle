...
use timekettle_forest::{root_for, RootResolution};
...
pub enum TapeItem {
    ...
    Event(Box<timekettle::BoundaryEvent>),
    /// A line that did not parse as a `TimekettleRecord`.
    Malformed {
        line_no: usize,
        error: String,
        excerpt: String,
...
impl ScopedRecording {
        ...
        Ok(reader
            ...
            .filter_map(move |(i, line)| {
                ...
                match serde_json::from_str::<timekettle::TimekettleRecord>(&line) {
                    Ok(timekettle::TimekettleRecord::BoundaryEvent(event)) => self
                        .scope
                        .contains(event.correlation_id.as_deref())
                        .then_some(TapeItem::Event(event)),
                    Ok(timekettle::TimekettleRecord::GraphNode(_) | timekettle::TimekettleRecord::Observed(_)) => None,
    ...
    pub fn graph_nodes(&self) -> io::Result<Vec<ExecutionGraphNode>> {
        ...
        self.for_each_line(|_, line| {
            if let Ok(timekettle::TimekettleRecord::GraphNode(node)) =
    ...
    fn index(&self) -> io::Result<TapeIndex> {
        ...
        self.for_each_line(|_, line| {
            // Cheap discriminator first: full `TimekettleRecord` parse on 171,234
...
/// pass 1 needs. Deserializing the full `TimekettleRecord` to read `parent_id` would
...
impl TapeSlot {
    ...
    /// uses — a full `TimekettleRecord` parse per line to read one field is the cost
    /// this shape exists to avoid.
    ///
    /// The sealed session's correlation index (`timekettle-compactor`) is ordered the

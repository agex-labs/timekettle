...
//! `timekettle-runtime` (`addresses_for`, `canonical_args_hash`, `KeyStamper`); this
...
use timekettle::{addresses_for, canonical_args_hash, KeyStamper, LookupEntry, LookupTable};
    ...
    fn renderer_skips_http_incoming_and_emits_one_entry_per_rank() {
        ...
        assert!(
            table.entries.iter().any(|e| matches!(
                &e.key.address,
                timekettle::Address::Sequence { boundary, .. } if boundary == "redis"
            )),

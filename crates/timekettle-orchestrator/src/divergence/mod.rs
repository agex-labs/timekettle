...
use timekettle::{Address, LocalFileLookupSource, LookupTable, LookupTableSource, ObservedCall};
use timekettle_kernel::{HttpDiff, JsonFieldDiff};
...
pub struct CorrelationOutcome {
    ...
    pub side_effect_divergences: u64,
    pub scoring_mode: timekettle_forest::ScoringMode,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub alignment: Option<timekettle_forest::Alignment>,
...
pub struct RunArtifacts {
    ...
    pub record_graph: Option<Vec<timekettle_core::ExecutionGraphNode>>,
    /// Raw replay-side execution graph, in observed-stream order.
    pub replay_graph: Vec<timekettle_core::ExecutionGraphNode>,
    ...
    pub events: Vec<timekettle::BoundaryEvent>,
...
struct GraphCorrelationPlan {
    ...
    scoring_mode: timekettle_forest::ScoringMode,
    alignment: Option<timekettle_forest::Alignment>,
    record: Option<timekettle_forest::ActivationForest>,
    replay: Option<timekettle_forest::ActivationForest>,
...
impl GraphScoringPlan {
        ...
        for correlation_id in correlation_ids {
            let record_build = art.record_graph.as_ref().map(|nodes| {
                ...
                let events: Vec<_> = art
                    ...
                    .map(|event| timekettle_forest::EventRef {
                    ...
                    .collect();
                timekettle_forest::build(&nodes, &events).inspect(|set| {
            ...
            let replay_events: Vec<_> = art
                ...
                .map(|(index, observed)| timekettle_forest::EventRef {
                ...
                .collect();
            let replay_build = timekettle_forest::build(&replay_nodes, &replay_events).inspect(|set| {
            ...
            let cycle = record_build
                ...
                || replay_build
                    ...
                    .is_some_and(|set| set.unusable.contains_key(&correlation_id));
            let reason = if cycle {
                Some(timekettle_forest::FlatReason::CycleDetected)
            } else if record.is_none() || replay.is_none() {
                Some(timekettle_forest::FlatReason::MissingForest)
                ...
                Some(timekettle_forest::FlatReason::IngressRootAsymmetry)
            } else {
                None
            };
            ...
            let (scoring_mode, alignment, flat_replay_nodes, flat_record_nodes) =
                if let Some(reason) = reason {
                    (
                        timekettle_forest::ScoringMode::Flat { reason },
                        None,
                        HashSet::new(),
                        HashSet::new(),
                    )
                    ...
                    let mut alignment = timekettle_forest::align(record_forest, replay_forest);
                    alignment
                        .flat_tier_events
                    ...
                    let served: BTreeMap<u64, u64> = replay_forest
                        ...
                        .collect();
                    timekettle_forest::reconcile_serving(&mut alignment, &served, record_forest);
                    ...
                    (
                        timekettle_forest::ScoringMode::Graph,
                        Some(alignment),
                        flat_replay_nodes,
                        flat_record_nodes,
                    )
    ...
    pub(crate) fn mode(&self, correlation_id: &str) -> Option<&timekettle_forest::ScoringMode> {
        self.correlations
            .get(correlation_id)
            .map(|entry| &entry.scoring_mode)
    }

    pub(crate) fn alignment(&self, correlation_id: &str) -> Option<&timekettle_forest::Alignment> {
        self.correlations
            .get(correlation_id)
    ...
    pub(crate) fn is_graph(&self, correlation_id: Option<&str>) -> bool {
        correlation_id
            ...
            .is_some_and(|mode| matches!(mode, timekettle_forest::ScoringMode::Graph))
    }
    ...
    pub(crate) fn alignment_row_for_replay_node(
        ...
        correlation_id: &str,
        replay_node: u64,
    ) -> Option<&timekettle_forest::AlignedNode> {
    ...
    pub(crate) fn alignment_row_uses_flat_tier(
        &self,
        correlation_id: &str,
        row: &timekettle_forest::AlignedNode,
    ) -> bool {
        ...
        row.record_node
            .map(|node| {
                self.recorded_event_sequences(
                    correlation_id,
                    node,
                    matches!(&row.outcome, timekettle_forest::NodeOutcome::PrunedSubtree { .. }),
                )
            })
            .unwrap_or_default()
    }
        ...
        row.replay_node
            .map(|node| {
                self.replay_event_indices(
                    correlation_id,
                    node,
                    matches!(&row.outcome, timekettle_forest::NodeOutcome::NovelSubtree { .. }),
                )
            })
            .unwrap_or_default()
    }
    ...
    fn scored_alignment(
    ...
    ) -> Option<timekettle_forest::Alignment> {
        let entry = self.correlations.get(correlation_id)?;
        ...
        for row in &mut alignment.nodes {
            ...
            if !divergent_nodes.contains(&replay_node)
                || matches!(&row.outcome, timekettle_forest::NodeOutcome::IdentitySkew { .. })
            {
                continue;
            }
            ...
            row.outcome = timekettle_forest::NodeOutcome::ValueDiverged { origin };
        }
        Some(alignment)
    }
}

fn event_bearing_ingress_root(forest: &timekettle_forest::ActivationForest) -> bool {
    forest.roots.iter().any(|root| {
        let node = &forest.nodes[root];
        node.span_name == "timekettle::http_incoming" && node.subtree_events > 0
    })
}

fn forest_event_sequences(
    forest: &timekettle_forest::ActivationForest,
    root: u64,
    include_subtree: bool,
) -> Vec<u64> {
...
fn graph_identity_skew(
    ...
    match &row.outcome {
        timekettle_forest::NodeOutcome::IdentitySkew {
            aligned_event,
            served_event,
...
///
/// Conservative in the shape of `timekettle::db::binds_read_keys`: anything that does
...
pub(crate) fn correlation_column_provenance(
    events: &[timekettle::BoundaryEvent],
    observed: &[ObservedCall],
) -> CorrelationColumnProvenance {
    let mut provenance = CorrelationColumnProvenance::default();
    let events_by_seq: HashMap<u64, &timekettle::BoundaryEvent> = events
        .iter()
...
pub(crate) fn observed_schema_default_divergence(
    obs: &ObservedCall,
    event: Option<&timekettle::BoundaryEvent>,
) -> SchemaDefaultVerdict {
...
fn resolve_canon(canon: Option<&timekettle::CanonRef>) -> Option<CanonPreset> {
...
fn event_state_canon(ev: &timekettle::BoundaryEvent) -> Option<CanonPreset> {
    resolve_canon(ev.declaration.as_ref()?.state_canon.as_ref())
}

fn event_reply_canon(ev: &timekettle::BoundaryEvent) -> Option<CanonPreset> {
    resolve_canon(ev.declaration.as_ref()?.reply_canon.as_ref())
}

pub(crate) fn event_reply_canon_kind(ev: &timekettle::BoundaryEvent) -> Option<String> {
    event_reply_canon(ev).map(|canon| canon.preset_name().to_owned())
}

fn event_value_canon(ev: &timekettle::BoundaryEvent) -> Option<CanonPreset> {
...
pub(crate) fn observed_value_diverged(
    ...
    obs.resolved
        && obs.provenance == timekettle::Provenance::Shadow
...
fn pairing_shape(
    ...
    // Fields timekettle's own args contract defines as WHAT KIND of call this is,
...
impl<'a> ArgsFreePairing<'a> {
    ...
    pub(crate) fn build(
        table: &LookupTable,
        events: &'a [timekettle::BoundaryEvent],
        provenance: &'a CorrelationColumnProvenance,
    ) -> Self {
        let events_by_seq: HashMap<u64, &timekettle::BoundaryEvent> =
...
/// lineage bucket minted by the correlation layer for a `timekettle.fork` span. Such
...
fn declared_idempotent_delete(ev: &timekettle::BoundaryEvent) -> Option<bool> {
    let declaration = ev.declaration.as_ref()?;
    let effect = declaration.effect?;
    let op = declaration.op?;
    Some(effect == timekettle::EffectKind::Redis && op == timekettle::OperationKind::IdempotentDelete)
}
...
fn db_row_key_from_state_key(raw: &str) -> Option<DbRowKey> {
    let parsed = timekettle::StateKey::parse(raw).ok()?;
    let wire = parsed.to_wire();
    let table = parsed.db_table()?.to_owned();
    match parsed {
        timekettle::StateKey::DbRow { key, .. } => Some(DbRowKey { table, key, wire }),
        _ => None,
    }
}
...
fn http_incoming_events_by_correlation(
    events: &[timekettle::BoundaryEvent],
) -> HashMap<String, &timekettle::BoundaryEvent> {
    events
        .iter()
...
fn http_diff_absorbed_by_reply_canon(
    diff: &HttpDiff,
    recorded_http: Option<&timekettle::BoundaryEvent>,
    body: &JsonFieldDiff,
) -> bool {
...
fn lineage_bucket(ev: &timekettle::BoundaryEvent) -> Option<&str> {
    ev.bucket_id
        .as_deref()
...
fn unordered_distinct_lineage(
    a: &timekettle::BoundaryEvent,
    b: &timekettle::BoundaryEvent,
...
fn event_windows_overlap(a: &timekettle::BoundaryEvent, b: &timekettle::BoundaryEvent) -> bool {
...
fn is_request_teardown_marker(ev: &timekettle::BoundaryEvent) -> bool {
    ev.boundary == "redis"
...
fn tail_gap_evidence(
    ...
    let mut last_recorded: HashMap<&str, &timekettle::BoundaryEvent> = HashMap::new();
    for ev in events {
...
fn http_clean_by_correlation(
    http_diffs: &[HttpDiff],
    http_incoming_by_correlation: &HashMap<String, &timekettle::BoundaryEvent>,
...
pub(crate) fn detect_with_plan(art: &RunArtifacts, graph_plan: &GraphScoringPlan) -> Scorecard {
    // V1: uncorrelated (background-task) events are tolerated; the timekettle-tokio
    ...
    let events_by_seq: HashMap<u64, &timekettle::BoundaryEvent> = art
        .events
        .iter()
    ...
    for (corr, (status_match, body_match)) in &corr_http {
        ...
        per_correlation.push(CorrelationOutcome {
            ...
            scoring_mode: graph_plan.mode(corr).cloned().unwrap_or(
                timekettle_forest::ScoringMode::Flat {
                    reason: timekettle_forest::FlatReason::MissingForest,
                },
            ),
...
fn stream_timekettle_records(
    path: &std::path::Path,
    warnings: &mut Vec<String>,
    mut route: impl FnMut(timekettle::TimekettleRecord),
) -> bool {
    ...
    for (i, line) in io::BufReader::new(file).lines().enumerate() {
        ...
        match serde_json::from_str::<timekettle::TimekettleRecord>(&line) {
...
fn load_replay_stream(
    path: &std::path::Path,
    warnings: &mut Vec<String>,
) -> (Vec<ObservedCall>, Vec<timekettle_core::ExecutionGraphNode>) {
    let mut observed = Vec::new();
    let mut graph = Vec::new();
    if !stream_timekettle_records(path, warnings, |record| match record {
        timekettle::TimekettleRecord::Observed(call) => observed.push(*call),
        timekettle::TimekettleRecord::GraphNode(node) => graph.push(*node),
        timekettle::TimekettleRecord::BoundaryEvent(_) => {}
    }) {
...
fn load_record_graph(
    path: &std::path::Path,
    warnings: &mut Vec<String>,
) -> Option<Vec<timekettle_core::ExecutionGraphNode>> {
    let mut graph = Vec::new();
    stream_timekettle_records(path, warnings, |record| {
        if let timekettle::TimekettleRecord::GraphNode(node) = record {
            graph.push(*node);
        }
    })
    .then_some(graph)
...
mod tests {
    ...
    use super::*;
    use timekettle::{LookupEntry, LookupKey};
    use timekettle_kernel::JsonFieldDiff;
    ...
    fn canon_presets_resolve_and_compare_their_declared_shapes() {
        let final_state = resolve_canon(Some(&timekettle::CanonRef::new("final_state")))
        ...
        let absent_after = resolve_canon(Some(&timekettle::CanonRef::new("absent_after")))
        ...
        let project = resolve_canon(Some(&timekettle::CanonRef::new("project:result,kind")))
    ...
    fn obs(
        ...
        ObservedCall {
            ...
            provenance: timekettle::Provenance::default(),
            seed_gap: false,
        }
    }
    ...
    fn build_ledger_never_attaches_a_recorded_side_from_outside_the_run_scope() {
        ...
        write_jsonl_rows(
            ...
            &[timekettle::TimekettleRecord::Observed(Box::new(exec_obs(
                "db",
                Some("c-keep"),
                true,
                Some(2),
    ...
    fn write_recording_tape(path: &std::path::Path, events: &[timekettle::BoundaryEvent]) {
        let rows: Vec<timekettle::TimekettleRecord> = events
            .iter()
            .cloned()
            .map(|event| timekettle::TimekettleRecord::BoundaryEvent(Box::new(event)))
            .collect();
    ...
    fn exec_obs(
        ...
        o.provenance = timekettle::Provenance::Shadow;
    ...
    fn graph_node(node_id: u64, correlation_id: Option<&str>) -> timekettle_core::ExecutionGraphNode {
        timekettle_core::ExecutionGraphNode {
            node_id,
    ...
    fn load_artifacts_carries_both_execution_graph_streams() {
        ...
        let replay_rows = vec![
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(11, Some("c1")))),
            timekettle::TimekettleRecord::Observed(Box::new(obs("db", Some("c1"), true, Some(3), Some(1)))),
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(12, Some("c1")))),
        ];
        let record_rows = vec![
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(1, Some("c1")))),
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(2, Some("c1")))),
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(3, Some("c1")))),
        ];
    ...
    fn unavailable_record_graph_preserves_detection_and_distinguishes_present_empty() {
        ...
        write_jsonl_rows::<timekettle::TimekettleRecord>(&root.record_graph_path(run_id), &[]);
    ...
    fn graph_streams_follow_filtered_run_scope_and_keep_ambient_nodes() {
        ...
        let rows = |offset| {
            vec![
                timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(offset, Some("c-keep")))),
                timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(offset + 1, Some("c-drop")))),
                timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(offset + 2, None))),
            ]
        };
    ...
    fn interleaved_replay_records_split_in_source_order_in_one_pass() {
        ...
        let rows = vec![
            timekettle::TimekettleRecord::Observed(Box::new(obs("first", Some("c1"), true, None, Some(1)))),
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(21, Some("c1")))),
            timekettle::TimekettleRecord::Observed(Box::new(obs("second", Some("c1"), true, None, Some(2)))),
            timekettle::TimekettleRecord::GraphNode(Box::new(graph_node(22, Some("c1")))),
        ];
        write_jsonl_rows(&path, &rows);
        let mut warnings = Vec::new();
    ...
    fn art_with_events(
        ...
        observed: Vec<ObservedCall>,
        http: Vec<HttpDiff>,
        events: Vec<timekettle::BoundaryEvent>,
    ) -> RunArtifacts {
    ...
    fn body_pair(baseline: serde_json::Value, candidate: serde_json::Value) -> HttpDiff {
        ...
        let body_diff = timekettle_kernel::diff_json(&baseline, &candidate, "$", &[]);
    ...
    fn a_declared_bag_reply_canon_absorbs_a_reordering() {
        ...
        let card = detect(&art_with_events(
            ...
            vec![http_with_bodies(
                corr,
                true,
                timekettle_kernel::diff_json(&recorded, &replayed, "$", &[]),
                recorded.clone(),
                replayed,
            )],
    ...
    fn http_incoming_ev_with_reply_canon(
        ...
        ev.declaration = reply_canon.map(|canon| {
            timekettle::BoundaryDeclaration::default().reply_canon(timekettle::CanonRef::new(canon))
        });
        ev
    }
    ...
    fn project_canon_with_no_resolving_path_is_not_a_value_match() {
        let canon = resolve_canon(Some(&timekettle::CanonRef::new("project:settlement.state")))
            .expect("project preset resolves");
        assert!(
            !canon.equivalent(
        ...
        let resolving = resolve_canon(Some(&timekettle::CanonRef::new("project:status")))
            .expect("project preset resolves");
        assert!(
    ...
    fn db_update_ev(
        ...
        let write_set = timekettle::db::row_state_keys(table, &result)
            .into_iter()
            .map(|key| key.to_wire())
            .collect::<Vec<_>>();
    ...
    fn declared_db_update_ev(
        ...
        ev.declaration = Some(
            timekettle::BoundaryDeclaration::default()
                .effect(timekettle::EffectKind::Db)
                .operation(timekettle::OperationKind::Update)
                .returns(timekettle::ReturnSemantics::UpdateReturning),
        );
        ev
    }
    ...
    fn db_read_ev(
        ...
        let result = envelope(row);
        let read_set = timekettle::db::row_state_keys(table, &result)
            .into_iter()
            .map(|key| key.to_wire())
            .collect::<Vec<_>>();
        serde_json::from_value(serde_json::json!({
    ...
    fn redis_delete_ev_with_reply_canon(
        ...
        let declaration = ev
            ...
            .reply_canon(timekettle::CanonRef::new(canon));
        ev.declaration = Some(declaration);
        ev
    }
    ...
    fn rule_b_demotes_declared_renamed_idempotent_delete() {
        let card = detect(&art_with_events(
            ...
            vec![redis_delete_ev(
                ...
                101,
                "remove_cache_entry",
                timekettle::OperationKind::IdempotentDelete,
            )],
        ));
    ...
    fn rule_b_declared_non_idempotent_delete_stays_blocking() {
        let card = detect(&art_with_events(
            ...
            vec![redis_delete_ev(
                ...
                101,
                "delete_key",
                timekettle::OperationKind::Delete,
            )],
        ));
    ...
    fn omitted_ev(seq: u64, boundary: &str, corr: Option<&str>) -> timekettle::BoundaryEvent {
        serde_json::from_value(serde_json::json!({
            "global_sequence": seq,
            "request_sequence": 0,
            "correlation_id": corr,
    ...
    /// recording ends at.
    fn teardown_ev(seq: u64, corr: &str) -> timekettle::BoundaryEvent {
        tail_ev(
            seq,
            corr,
            "redis",
    ...
    fn with_graphs(
        ...
        record: Vec<timekettle_core::ExecutionGraphNode>,
        replay: Vec<timekettle_core::ExecutionGraphNode>,
    ) -> RunArtifacts {
    ...
    fn both_forests_choose_and_serialize_graph_mode() {
        ...
        assert_eq!(outcome.scoring_mode, timekettle_forest::ScoringMode::Graph);
        assert!(outcome
            .alignment
            .as_ref()
    ...
    fn one_sided_event_bearing_http_ingress_root_demotes_instead_of_misaligning() {
        ...
        let artifacts = with_graphs(
            ...
            vec![graph_span(3, corr, None, 0, "timekettle::http_incoming")],
        ...
        assert_eq!(
            outcome.scoring_mode,
            timekettle_forest::ScoringMode::Flat {
                reason: timekettle_forest::FlatReason::IngressRootAsymmetry
            }
        );
        assert!(outcome.alignment.is_none());
    }
    ...
    fn mixed_graph_and_flat_correlations_keep_weighted_accounting_independent() {
        ...
        let flat_outcome = card
            ...
            .unwrap();
        assert_eq!(graph_outcome.scoring_mode, timekettle_forest::ScoringMode::Graph);
        assert_eq!(
        ...
        assert_eq!(
            alignment
                ...
                .filter(|row| matches!(
                    &row.outcome,
                    timekettle_forest::NodeOutcome::PrunedSubtree { events_below: 2 }
                ))
                .count(),
            1
        );
        assert_eq!(
            alignment
                ...
                .filter(|row| matches!(
                    &row.outcome,
                    timekettle_forest::NodeOutcome::NovelSubtree { events_below: 1 }
                ))
                .count(),
            1
        );
    ...
    fn graph_alignment_records_value_difference_before_schema_demotion() {
        ...
        assert_eq!(
            aligned.outcome,
            timekettle_forest::NodeOutcome::ValueDiverged { origin: true },

...
use timekettle::{Address, BoundaryEvent, ObservedCall};
...
fn observed_side(obs: &ObservedCall) -> CallSide {
    ...
    let result = if obs.provenance == timekettle::Provenance::Shadow
...
pub(crate) fn build_with_plan(
    ...
    for correlation_id in graph_correlations {
        ...
        for alignment_row in &alignment.nodes {
            use timekettle_forest::NodeOutcome;
...
/// `expected` keying.
pub fn expected_sequences(table: &timekettle::LookupTable) -> HashSet<u64> {
    table
        .entries
        .iter()
...
pub fn recorded_span_paths(table: &timekettle::LookupTable) -> HashMap<u64, String> {
...
mod tests {
    ...
    fn event(seq: u64, boundary: &str, corr: Option<&str>) -> BoundaryEvent {
        BoundaryEvent {
            ...
            fidelity: timekettle::Fidelity::default(),
            result_image: None,
            pre_image: None,
            read_set: Vec::new(),
            write_set: Vec::new(),
            value_digest: None,
            entropy_source: None,
            replay_strategy: timekettle::ReplayStrategy::default(),
            kind: None,
            declaration: None,
            raw_draw: None,
    ...
    fn table_for(events: &[BoundaryEvent], spans: &HashMap<u64, String>) -> timekettle::LookupTable {
        let mut entries = Vec::new();
        for ev in events {
            let key = |address| timekettle::LookupKey {
                correlation_id: ev.correlation_id.clone(),
            ...
            entries.push(timekettle::LookupEntry {

...
//!
//! Every settle goes through [`timekettle_store::Store::update_run_state`], which is
...
/// How often the reconciler runs a pass (override: `TIMEKETTLE_RECONCILE_INTERVAL_SECS`).
const DEFAULT_INTERVAL_SECS: u64 = 30;

/// How long a non-terminal run with NO Job is tolerated before it is failed as
/// orphaned (override: `TIMEKETTLE_RECONCILE_ORPHAN_GRACE_SECS`). Generous enough to
...
pub fn spawn(store: Arc<Store>, incluster: InClusterConfig, cfg: K8sExecutorConfig) {
    let transport = match UreqTransport::new(&incluster) {
        ...
        Err(e) => {
            eprintln!(
                "timekettle-orchestrator: k8s reconciler NOT started — cannot build kube client: {e}"
            );
            return;
        }
    };
    ...
    let interval = Duration::from_secs(env_secs(
        "TIMEKETTLE_RECONCILE_INTERVAL_SECS",
        DEFAULT_INTERVAL_SECS,
    ));
    let grace = Duration::from_secs(env_secs(
        "TIMEKETTLE_RECONCILE_ORPHAN_GRACE_SECS",
        DEFAULT_ORPHAN_GRACE_SECS,
    ));
    eprintln!(
        "timekettle-orchestrator: k8s reconciler started (jobs ns {}, every {}s, orphan grace {}s)",
        cfg.jobs_namespace,
        interval.as_secs(),
        grace.as_secs()
    );
...
mod tests {
    ...
    fn job(run_id: &str, verdict: Option<bool>) -> RunJob {
        RunJob {
            job_name: format!("timekettle-replay-{run_id}"),
            run_id: run_id.to_owned(),
            verdict,
        }
    }
    ...
    fn run_jobs_from_items_extracts_run_id_and_verdict() {
        let items = vec![
            json!({
                "metadata": { "name": "timekettle-replay-run-1", "labels": { "timekettle.run-id": "run-1" } },
                "status": { "conditions": [{ "type": "Complete", "status": "True" }] }
            }),
            json!({
                "metadata": { "name": "timekettle-replay-run-2", "labels": { "timekettle.run-id": "run-2" } },
                "status": { "active": 1 }
            }),
        ];
        let jobs = run_jobs_from_items(&items, RUN_ID_LABEL);
        ...
        assert_eq!(jobs[0].job_name, "timekettle-replay-run-1");

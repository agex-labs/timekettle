...
pub fn apply_job_patch(template: &Value, patch: &JobPatch) -> Result<Value, PatchError> {
    ...
    // differs from recording — the stores are the Job's own sidecars, timekettle runs in
...
mod tests {
        ...
        json!({
            ...
            "spec": {
                "template": {
                    ...
                    "spec": {
                        "containers": [
                            { "name": "runner", "image": "timekettle-orchestrator:template",
                              "env": [ { "name": "TIMEKETTLE_RUN_ID", "value": "PLACEHOLDER" } ] },
    ...
    fn config_env_copy_lands_verbatim_and_run_env_wins() {
        ...
        let patch = JobPatch {
            ...
            env: vec![EnvUpsert::new("candidate", "ROUTER__TIMEKETTLE__MODE", "replay")],
            ..Default::default()
        };
        ...
        assert_eq!(
            env.iter()
                .find(|e| e["name"] == json!("ROUTER__TIMEKETTLE__MODE"))
    ...
    fn patches_name_labels_image_and_upserts_env() {
        let patch = JobPatch {
            job_name: "timekettle-replay-run7".into(),
            labels: vec![("timekettle.run-id".into(), "run7".into())],
            images: vec![("candidate".into(), "candidate:sha_c".into())],
            env: vec![
                // upsert existing
                EnvUpsert::new("runner", "TIMEKETTLE_RUN_ID", "run7"),
                // append new
        ...
        assert_eq!(out["metadata"]["name"], json!("timekettle-replay-run7"));
        assert_eq!(out["metadata"]["labels"]["timekettle.run-id"], json!("run7"));
        ...
        assert_eq!(
            out["spec"]["template"]["metadata"]["labels"]["timekettle.run-id"],
            json!("run7")
        );
        ...
        // TIMEKETTLE_RUN_ID replaced in place, not duplicated.
        let runner_env = runner["env"].as_array().expect("runner env array");
        let run_ids: Vec<_> = runner_env
            .iter()
            .filter(|e| e["name"] == json!("TIMEKETTLE_RUN_ID"))
            .collect();
        ...
        assert_eq!(candidate["env"][0]["name"], json!("ROUTER__TIMEKETTLE__MODE"));
    ...
    fn env_can_target_an_init_container() {
        ...
        assert_eq!(inits[0]["env"][0]["name"], json!("TIMEKETTLE_CODE_BUNDLE_URI"));
        assert_eq!(
            inits[0]["env"][0]["value"],

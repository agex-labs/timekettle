...
//! ONCE at startup. This is the single canonical home for `TIMEKETTLE_EXECUTOR` (the
...
impl ExecutorKind {
    /// Read `TIMEKETTLE_EXECUTOR` (`compose` | `k8s`), defaulting to compose. An
...
impl K8sExecutorConfig {
    pub fn from_env() -> Self {
        ...
        let candidate_container = var("TIMEKETTLE_CANDIDATE_CONTAINER", "candidate");
        K8sExecutorConfig {
            jobs_namespace: var("TIMEKETTLE_JOBS_NAMESPACE", "replay-sbx"),
            template_namespace: var("TIMEKETTLE_JOB_TEMPLATE_NAMESPACE", "replay-env"),
            template_configmap: var("TIMEKETTLE_JOB_TEMPLATE_CONFIGMAP", "job-template"),
            template_key: var("TIMEKETTLE_JOB_TEMPLATE_KEY", "job.json"),
            runner_container: var("TIMEKETTLE_RUNNER_CONTAINER", "runner"),
            migrations_init_container: var("TIMEKETTLE_MIGRATIONS_INIT_CONTAINER", "migrations"),
            config_init_container: var("TIMEKETTLE_CONFIG_INIT_CONTAINER", "router-config"),
            code_bundle_uri_env: var("TIMEKETTLE_CODE_BUNDLE_URI_ENV", "TIMEKETTLE_CODE_BUNDLE_URI"),
            job_state_dir: var("TIMEKETTLE_JOB_STATE_DIR", "/workspace/state"),
            diagnostics_tail_lines: var("TIMEKETTLE_DIAGNOSTICS_TAIL_LINES", "2000")
                .parse()
                .unwrap_or(2000),
            ...
            candidate_binding: CandidateBinding {
                ...
                mode_env: var("TIMEKETTLE_CANDIDATE_MODE_ENV", "ROUTER__TIMEKETTLE__MODE"),
                run_id_env: var("TIMEKETTLE_CANDIDATE_RUN_ID_ENV", "ROUTER__TIMEKETTLE__RUN_ID"),
                source_env: var("TIMEKETTLE_CANDIDATE_SOURCE_ENV", "ROUTER__TIMEKETTLE__REPLAY__SOURCE"),
                observed_env: var(
                    "TIMEKETTLE_CANDIDATE_OBSERVED_ENV",
                    "ROUTER__TIMEKETTLE__REPLAY__OBSERVED_SINK",
                ),
                code_sha_env: var(
                    "TIMEKETTLE_CANDIDATE_CODE_SHA_ENV",
                    "ROUTER__TIMEKETTLE__IDENTITY__CODE_SHA",
                ),
            },
            ...
            config_source: ConfigSource {
                deployment: var(
                    "TIMEKETTLE_CONFIG_SOURCE_DEPLOYMENT",
                    "replay-sbx-hyperswitch-server",
                ),
                container: var("TIMEKETTLE_CONFIG_SOURCE_CONTAINER", "hyperswitch-router"),
            },
        }
    }
}
...
/// lives, from `TIMEKETTLE_CANDIDATE_IMAGE_REPO`.
///
...
fn qualify_candidate_image(reference: &str) -> String {
    ...
    let repo = match std::env::var("TIMEKETTLE_CANDIDATE_IMAGE_REPO") {
...
mod tests {
    ...
    fn executor_kind_defaults_to_compose_when_unset() {
        // TIMEKETTLE_EXECUTOR is unset in the test env → compose default, no error.
...
mod candidate_reference_tests {
    ...
    /// `TIMEKETTLE_CANDIDATE_IMAGE_REPO` is process-global, so these run under one
    ...
    fn a_bare_ref_resolves_against_the_deployment_registry() {
        ...
        std::env::set_var("TIMEKETTLE_CANDIDATE_IMAGE_REPO", REPO);
        ...
        std::env::remove_var("TIMEKETTLE_CANDIDATE_IMAGE_REPO");
        assert_eq!(
            qualify_candidate_image("timekettle-router-local"),
            "timekettle-router-local"
        );
    }
}

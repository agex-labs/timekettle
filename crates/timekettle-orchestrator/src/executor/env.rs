...
/// artifact from. For the Hyperswitch router these are the `ROUTER__TIMEKETTLE__*`
...
pub fn runner_env(
    ...
    let mut env = vec![
        EnvUpsert::new(container, "TIMEKETTLE_RUN_ID", run_id),
        EnvUpsert::new(container, "TIMEKETTLE_RUN_SPEC", run_spec_json),
    ];
...
mod tests {
    ...
    fn binding() -> CandidateBinding {
        CandidateBinding {
            ...
            mode_env: "ROUTER__TIMEKETTLE__MODE".into(),
            run_id_env: "ROUTER__TIMEKETTLE__RUN_ID".into(),
            source_env: "ROUTER__TIMEKETTLE__REPLAY__SOURCE".into(),
            observed_env: "ROUTER__TIMEKETTLE__REPLAY__OBSERVED_SINK".into(),
            code_sha_env: "ROUTER__TIMEKETTLE__IDENTITY__CODE_SHA".into(),
        }
    }
    ...
    fn candidate_env_maps_artifacts_to_the_bound_vars() {
        let root =
            HarnessRoot::new(std::env::temp_dir().join("timekettle-test-env-binding")).expect("root");
        let contract = root.replay_contract("run-5");
        let env = binding().env_for(&contract, "abc123");

        assert_eq!(find(&env, "ROUTER__TIMEKETTLE__MODE").value, "replay");
        assert_eq!(find(&env, "ROUTER__TIMEKETTLE__RUN_ID").value, "run-5");
        assert_eq!(
            find(&env, "ROUTER__TIMEKETTLE__REPLAY__SOURCE").value,
            contract.lookup_table.display().to_string()
        );
        assert_eq!(
            find(&env, "ROUTER__TIMEKETTLE__REPLAY__OBSERVED_SINK").value,
            contract.observed_sink.display().to_string()
        );
        assert_eq!(
            find(&env, "ROUTER__TIMEKETTLE__IDENTITY__CODE_SHA").value,
            "abc123"
        );
    ...
    fn runner_env_includes_expected_migrations_only_when_supplied() {
        ...
        assert_eq!(find(&with, "TIMEKETTLE_RUN_ID").value, "run-5");

...
pub const RUN_ID_LABEL: &str = "timekettle.run-id";
...
pub fn job_name_for(run_id: &str) -> String {
    let mut s: String = format!("timekettle-replay-{run_id}")
        .chars()
        .map(|c| {
...
mod tests {
    ...
    fn spec() -> LaunchSpec {
        LaunchSpec {
            ...
            env: vec![
                EnvUpsert::new("runner", "TIMEKETTLE_RUN_ID", "run-9f"),
                EnvUpsert::new("candidate", "ROUTER__TIMEKETTLE__MODE", "replay"),
            ],
            labels: vec![("timekettle.run-id".into(), "run-9f".into())],
            config_source: None,
        }
    }
    ...
    /// observed sink '/var/lib/timekettle/state/observed/<run>.jsonl': Permission denied`.
    ...
    fn kill_deletes_the_job_and_sweeps_remaining_pods() {
        ...
        assert_eq!(r.job_deleted.as_deref(), Some("timekettle-replay-run-7"));
    ...
    fn diagnostics_report_container_state_and_output() {
        let pod = json!({ "items": [ { "metadata": { "name": "timekettle-replay-run-7-abc" },
            "status": {
                "phase": "Failed",
    ...
    fn build_job_copies_recorded_env_and_checks_its_secrets() {
        ...
        assert!(env
            .iter()
            .any(|e| e["name"] == json!("ROUTER__TIMEKETTLE__MODE") && e["value"] == json!("replay")));
    ...
    fn build_job_fetches_template_and_applies_patch() {
        ...
        assert_eq!(job["metadata"]["name"], json!("timekettle-replay-run-9f"));
        assert_eq!(job["metadata"]["labels"]["timekettle.run-id"], json!("run-9f"));
        let containers = job["spec"]["template"]["spec"]["containers"]
            .as_array()
            .expect("containers");
        assert_eq!(containers[1]["image"], json!("hyperswitch:sha_c"));
        assert_eq!(containers[0]["env"][0]["name"], json!("TIMEKETTLE_RUN_ID"));
        assert_eq!(containers[1]["env"][0]["name"], json!("ROUTER__TIMEKETTLE__MODE"));
    }
    ...
    fn launch_create_conflict_is_idempotent_success() {
        ...
        assert_eq!(name, "timekettle-replay-run-9f");
    }
    ...
    fn job_name_is_dns_1123_safe() {
        assert_eq!(job_name_for("run_9F.x"), "timekettle-replay-run-9f-x");

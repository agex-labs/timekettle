...
fn run_id_for(spec: &RunSpec) -> String {
    ...
    replay_run_id(
        &std::env::var("TIMEKETTLE_ENV").unwrap_or_else(|_| "dev".to_owned()),
        &candidate,
        recording,
        &run_id_stamp(),
    )
...
/// Fill a `TIMEKETTLE_CANDIDATE_TARBALL_URL` template: `{sha}` always, `{repo}` from
/// the run's `candidate_repo` (a per-run parameter) or the `TIMEKETTLE_CANDIDATE_REPO`
...
fn resolve_tarball_url(template: &str, run_repo: Option<&str>, sha: &str) -> Option<String> {
    ...
    // it falls through to the TIMEKETTLE_CANDIDATE_REPO default — never a `//tar.gz`.
    let repo = run_repo
        .map(str::trim)
        .filter(|r| !r.is_empty())
        .map(str::to_owned)
        .or_else(|| std::env::var("TIMEKETTLE_CANDIDATE_REPO").ok())
        ...
        None => {
            eprintln!(
                "codebundle: tarball template needs {{repo}} but no candidate_repo / \
                 TIMEKETTLE_CANDIDATE_REPO for {sha}"
            );
            None
        }
    }
...
/// tried in order:
///   1. `TIMEKETTLE_CANDIDATE_TARBALL_URL` — a codeload-style `…/tar.gz/{sha}` template
...
///   2. `TIMEKETTLE_CANDIDATE_REPO_DIR` — a local candidate git checkout (compose/dev).
///
...
fn resolve_expected_schema(run: &Run) -> Option<(crate::SchemaFingerprint, String)> {
    ...
    // (candidate images can be built from any repo/fork); TIMEKETTLE_CANDIDATE_REPO is
    // only the orchestrator default.
    if let Some(url) = std::env::var("TIMEKETTLE_CANDIDATE_TARBALL_URL")
        .ok()
        .and_then(nonempty)
    ...
    if let Some(repo) = std::env::var("TIMEKETTLE_CANDIDATE_REPO_DIR")
        .ok()
        .and_then(nonempty)
    {
...
mod tests {
    ...
    fn tarball_url_blank_run_repo_is_ignored() {
        ...
        // Either None (no TIMEKETTLE_CANDIDATE_REPO in the env) or, if the env happens

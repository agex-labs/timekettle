...
mod tests {
    ...
    fn psql_script_runs_one_session_with_one_dash_c_per_command() {
        let commands = vec![
            "CREATE TEMP TABLE _timekettle_wire_seed AS SELECT \"a\" FROM \"t\" WITH NO DATA".to_string(),
            "\\copy _timekettle_wire_seed from pstdin with (format binary)".to_string(),
            "INSERT INTO \"t\" (\"a\") SELECT \"a\" FROM _timekettle_wire_seed ON CONFLICT DO NOTHING"
                .to_string(),
        ];

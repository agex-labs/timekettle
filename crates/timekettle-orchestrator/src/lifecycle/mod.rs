...
//! thin overlay (`docker-compose.timekettle.yml`) that swaps the router to a local
//! timekettle build and adds MinIO + a replay service; HS's kafka0 and vector are
...
//!   DEMO_COMPOSE_BASE    HS compose (default vendor/hyperswitch-timekettle-clean/docker-compose.yml)
//!   DEMO_COMPOSE_OVERLAY timekettle overlay (default demo/overlays/hyperswitch/docker-compose.timekettle.yml)
//!   DEMO_PROJECT         docker compose project name (default timekettle-demo)
//!   DEMO_REPLAY_PORT     host port for the replay candidate (default 8090; the
//!                        only host-published port — the host kernel hits it)
//!   DEMO_KERNEL_BIN      timekettle-kernel binary (default target/release/timekettle-kernel)
//!   DEMO_KAFKA_TOPIC     recording topic (default hyperswitch-timekettle-recording-events)
...
impl Demo {
    fn from_env(root: &HarnessRoot) -> Self {
        ...
        Self {
            compose_base: env(
                "DEMO_COMPOSE_BASE",
                "vendor/hyperswitch-timekettle-clean/docker-compose.yml",
            ),
            // The out-of-tree overlay (W4) is canonical: it carries the TYPED
            // ROUTER__TIMEKETTLE__* env the current router reads. The vendor tree
            // still holds a stale pre-typed copy — with that one, a candidate
            // silently boots with timekettle disabled.
            compose_overlay: env(
                "DEMO_COMPOSE_OVERLAY",
                "demo/overlays/hyperswitch/docker-compose.timekettle.yml",
            ),
            project: env("DEMO_PROJECT", "timekettle-demo"),
            replay_port: env("DEMO_REPLAY_PORT", "8090").parse().unwrap_or(8090),
            kernel_bin: env("DEMO_KERNEL_BIN", "target/release/timekettle-kernel"),
            topic: env("DEMO_KAFKA_TOPIC", "hyperswitch-timekettle-recording-events"),
    ...
    fn compose_env(&self, recording_id: &str, run_id: &str) -> Vec<(String, String)> {
        vec![
            ...
            ("TIMEKETTLE_RECORDING_TOPIC".into(), self.topic.clone()),
            ...
            (
                ...
                self.candidate_image
                    .clone()
                    .unwrap_or_else(|| "timekettle-router-local:latest".to_owned()),
            ),
            ...
            // the typed `timekettle.identity.git_sha_env` setting.
            (
                "VERGEN_GIT_SHA".into(),
    ...
    ///
    /// - project  → `timekettle-run-<last 8 alnum of run_id>`: a distinct compose
    ...
    ///   shared timekettle-demo project (record-side: kafka0, vector, minio, the
    ...
    fn isolated_for_replay(&self, run_id: &str) -> Self {
        ...
        out.project = format!("timekettle-run-{short}");
...
pub fn drive(root: &HarnessRoot, run_id: &str, ctx: &StoreCtx) {
    ...
    let outcome = match run.spec.mode {
        ...
        RunMode::Replay => {
            ...
            // stacks. Only for an ISOLATED project (never the shared timekettle-demo,
...
///   minimal docker context, bake `timekettle-candidate:<run8>`, and point compose at
...
fn resolve_candidate(
    ...
    let binary = match &run.spec.candidate_spec {
        CandidateSpec::PrebuiltImage { image } => {
            ...
            // "timekettle-demo" is the SPA's historical no-candidate default: the
            // legacy compose self-build (overlay default image, `--build`).
            // An empty ref means the same.
            if declared.is_empty() || declared == "timekettle-demo" {
                return Ok(());
            }
            // The SAME resolution the k8s executor uses (a bare ref qualifies
            // against TIMEKETTLE_CANDIDATE_IMAGE_REPO). One spec must not read as two
    ...
    let tag = format!("timekettle-candidate:{short}");
    let mut cmd = Command::new("docker");
...
fn drive_record(
    ...
    set_stage(
        ...
        3,
        total,
        "starting record router (TIMEKETTLE_MODE=record)",
    );
    compose_up(
        demo,
        ctx,
    ...
    // itself as `TimekettleRecord::GraphNode` lines, so the events artifact is the
    // whole recording.
    ctx.artifact(
        Some(&recording_id),
        "events",
...
fn drive_replay(
    ...
    set_stage(
        ...
        3,
        total,
        "starting replay router (TIMEKETTLE_MODE=replay)",
    );
    ...
    // concurrently rebuild the SAME `timekettle-router-local:latest` tag, racing the
    ...
    // TIMEKETTLE_MODE=replay, so the harness prepares concrete store state instead of
    ...
    // declarations plus TIMEKETTLE_MODE=replay. Seed materialization restores the
...
fn stage_resolve_recording(
    ...
    let recording_id = match &s3_source {
        ...
        // Session layout: the recording comes back out of the timekettle bucket.
...
/// execution-graph nodes (`TimekettleRecord::GraphNode`).
...
impl ArtifactSink {
    /// The in-pod runner selects the S3 sink via `TIMEKETTLE_RUN_ARTIFACT_S3=1` (set on
    /// the Job's runner container ONLY, never the orchestrator), using the
    /// runner's S3 config and `TIMEKETTLE_RUN_ARTIFACT_PREFIX` (default `replay-runs`).
    ...
    pub fn from_env() -> Self {
        if std::env::var("TIMEKETTLE_RUN_ARTIFACT_S3").ok().as_deref() == Some("1") {
            let prefix = std::env::var("TIMEKETTLE_RUN_ARTIFACT_PREFIX")
                .ok()
    ...
    pub fn publish(
        ...
        match self {
            ...
            ArtifactSink::S3 { cfg, prefix } => {
                ...
                match timekettle_compactor::put_object(cfg, &key, data) {
...
/// Extract the record-side execution-graph nodes (`TimekettleRecord::GraphNode`) for
...
fn write_record_graph_nodes(
    nodes: &[timekettle_core::ExecutionGraphNode],
    dest: &std::path::Path,
) -> Result<usize, String> {
    ...
    for node in nodes {
        ...
        let line = serde_json::to_string(&timekettle::TimekettleRecord::GraphNode(Box::new(node.clone())))
...
fn score_and_register(
    ...
    // execution-graph nodes (`TimekettleRecord::GraphNode`) — no separate artifact.
    ...
    let manifest = serde_json::json!({
        "schema": "timekettle.replay-run/v1",
        "run_id": run.run_id,
        "recording_id": recording_id,
...
///      router, `ROUTER__TIMEKETTLE__REPLAY__SOURCE`)
...
pub struct InPodOptions {
    ...
    pub router_port: u16,
    /// timekettle-kernel binary path inside the runner container.
...
pub fn drive_replay_in_pod(
    ...
    // In-pod: TIMEKETTLE_RUN_ARTIFACT_S3=1 (Job template) uploads artifacts to S3 so
...
/// replays don't leak stacks. A no-op when the project is the shared `timekettle-demo`
...
fn teardown_if_isolated(demo: &Demo, run_id: &str) {
    if !demo.project.starts_with("timekettle-run-") {
...
/// the [`timekettle::value::RedisWireValue`] transform (#39). One variant per value
...
/// [`timekettle::value::RedisWireValue::to_redis_string`]).
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl SeedCertificateEntry {
    ...
    fn not_a_precondition(
        ...
        boundary: &str,
        key: &str,
        reason: timekettle::NotPreconditionReason,
    ) -> Self {
        let why = match reason {
            timekettle::NotPreconditionReason::ReadAfterWrite => {
                "not a precondition: the read observes this correlation's own prior write of the key"
            }
            timekettle::NotPreconditionReason::SelfCreatedTable => {
        ...
        Self {
            ...
            physical_key: None,
            db_schema: None,
            origin: timekettle::SeedOrigin::Recording,
...
/// Build the total-derivative [`SeedPlan`](timekettle::SeedPlan) from the recording
...
/// recorded RESULTS (one [`build_seed_plan`](timekettle::build_seed_plan) per
...
/// logic lives in `timekettle-record` and is unit-tested without docker; this function
...
/// correlation's schema. Both are ON by default; `TIMEKETTLE_SEED_DB=0` kill-switches
...
fn materialize_seed_plan(
    ...
    // DB isolation + seeding is ON by default (R1: real seeding). `TIMEKETTLE_SEED_DB=0`
    ...
    let seed_db_enabled = std::env::var("TIMEKETTLE_SEED_DB")
        .ok()
        .map(|v| v.trim() != "0")
        .unwrap_or(true);
    ...
    for corr in &correlations {
        ...
        let plan = timekettle::build_seed_plan(&events, corr.as_deref()).with_ambient(&ambient);
        ...
        let db_schema = corr.as_deref().map(timekettle::db_schema_for);
        ...
        if seed_db_enabled {
            if let Some(schema) = db_schema.as_deref() {
                ...
                needed.extend(timekettle::build_write_target_tables(&events, corr.as_deref()));
        ...
        for entry in entries {
            match entry.boundary.as_str() {
                ...
                // default; TIMEKETTLE_SEED_DB=0 is the kill-switch.
                ...
                "db" => certificate.push(SeedCertificateEntry::new(
                    ...
                    db_schema.clone(),
                    SeedMaterializationStatus::Skipped,
                    SeedReadback::not_run("db seeding disabled by TIMEKETTLE_SEED_DB=0"),
                )),
...
fn seed_materialization_priority(entry: &timekettle::SeedEntry) -> u8 {
    if entry.boundary != "db" {
        return 0;
    }
    match timekettle::StateKey::parse(&entry.key) {
        Ok(timekettle::StateKey::DbRow { .. }) => 0,
        Ok(timekettle::StateKey::DbQuery { .. }) => 1,
        _ => 2,
    }
}
...
///
/// The table comes from typed v1 [`timekettle::StateKey`] state keys. Opaque/legacy DB
...
fn wire_copy_commands(qualified_table: &str, column_names: &[&str]) -> Vec<String> {
    let column_list = column_names
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let staging = "_timekettle_wire_seed";
    vec![
        format!(
...
fn wire_readback_row_predicate(row: &DbRowImage) -> Option<Vec<(String, String)>> {
    let object: serde_json::Map<String, serde_json::Value> = row
        ...
        .collect();
    match timekettle::db::row_state_key(&row.table, &serde_json::Value::Object(object)) {
        Some(timekettle::StateKey::DbRow { key, .. }) => Some(key),
        _ => None,
    }
}
...
fn db_seed_target_from_key(key: &str) -> Option<DbSeedTarget> {
    let state_key = match timekettle::StateKey::parse(key) {
    ...
    match &state_key {
        timekettle::StateKey::DbRow { key, .. } => Some(DbSeedTarget {
            table,
            kind: "row",
            row_filter: Some(DbRowFilter { key: key.clone() }),
        }),
        timekettle::StateKey::DbQuery { .. } => Some(DbSeedTarget {
            table,
            kind: "query-fallback",
            row_filter: None,
        }),
...
fn db_seed_value(envelope: &serde_json::Value) -> Option<serde_json::Value> {
    use timekettle::value::{TimekettleDatabaseResult, TimekettleDatabaseResultPayload};

    match serde_json::from_value::<TimekettleDatabaseResult>(envelope.clone()) {
        Ok(TimekettleDatabaseResult {
            payload: TimekettleDatabaseResultPayload::Ok { value, .. },
            ..
        }) => Some(value),
        Ok(TimekettleDatabaseResult {
            payload: TimekettleDatabaseResultPayload::Err { .. },
            ..
        }) => None,
...
/// Both halves of timekettle used to consult a hardcoded `table → column` map — the
...
fn register_table_identity(store: &StoreExec) {
    let mut command = store.psql(&["-A", "-t", "-F", "\t"], false, timekettle::TABLE_IDENTITY_SQL);
    ...
    timekettle::register_table_identity(by_table);
}
...
/// [`timekettle::SeedPlan::touched_db_tables`] and [`timekettle::build_write_target_tables`].
///
...
fn build_create_schema_sql(schema: &str, tables: &[String]) -> String {
    ...
    let clone_sql = if table_list.is_empty() {
        ...
        format!(
            "DO $timekettle$ DECLARE r record; BEGIN \
...
impl DbColumnMetadata {
    ...
    fn merge_typed(&self, typed: &timekettle::db::DbColumnImage) -> Self {
        Self {
...
/// `timekettle-diesel` connection wrapper and carried on the row image next to the
/// serde values (`timekettle::db::DbColumnImage::wire`). Consumed by seeding ONLY —
...
fn typed_column_has_metadata(column: &timekettle::db::DbColumnImage) -> bool {
...
fn typed_db_row_image(
    ...
    let payload: timekettle::db::DbRowImage = serde_json::from_value(value.clone()).ok()?;
    if payload.timekettle_image != timekettle::db::DbRowImage::KIND
        || payload.version != timekettle::db::DbRowImage::VERSION
...
fn wire_image_from_typed(payload: &timekettle::db::DbRowImage) -> Option<DbRowWireImage> {
    if payload.wire_format.as_deref() != Some(timekettle::db::DbRowImage::WIRE_FORMAT_BINARY) {
        return None;
    }
...
///
/// - **Recording-derived** — a serialized [`timekettle::value::RedisWireValue`], the
...
fn render_redis_seed_payload(value: &serde_json::Value, method: Option<&str>) -> RedisSeedRender {
    ...
    // text "Null".
    if let Ok(v) = serde_json::from_value::<timekettle::value::RedisWireValue>(value.clone()) {
...
/// [`timekettle::value::RedisWireValue`] — the compiler enforces the seeder answers
/// for every variant the codec can carry.
fn wire_value_seed_render(
    value: &timekettle::value::RedisWireValue,
    method: Option<&str>,
) -> RedisSeedRender {
    use timekettle::value::RedisWireValue as Wire;
    match value {
...
/// the shared `TimekettleRecord` stream exactly like the lookup renderer does.
...
fn read_scoped_events(recording: &crate::scope::ScopedRecording) -> Vec<timekettle::BoundaryEvent> {
...
///
/// If `TIMEKETTLE_AMBIENT_TEMPLATE` points at a `boundary\tkey\tvalue` TSV file, it is
/// parsed from there; otherwise the built-in EU-settlement
/// [`demo_defaults`](timekettle::AmbientTemplate::demo_defaults) supply the premium
/// rate — replacing the hand-coded `redis-cli SET settlement_rate_premium 0.20`.
fn load_ambient_template() -> timekettle::AmbientTemplate {
    if let Ok(path) = std::env::var("TIMEKETTLE_AMBIENT_TEMPLATE") {
        if !path.trim().is_empty() {
            match std::fs::read_to_string(&path) {
                Ok(text) => {
                    let template = timekettle::AmbientTemplate::from_tsv(&text);
                    eprintln!(
                ...
                Err(e) => {
                    eprintln!(
                        "lifecycle: could not read TIMEKETTLE_AMBIENT_TEMPLATE={path}: {e}; \
    ...
    timekettle::AmbientTemplate::demo_defaults()
}
...
/// streams the data parts (see `timekettle-compactor`). The ingest report and the
...
fn pull_recording(root: &HarnessRoot, ctx: &StoreCtx, recording_id: &str) -> Result<(), String> {
    ...
    if timekettle_compactor::read_manifest(&cfg, recording_id)?.is_none() {
        let root_prefix =
            std::env::var("TIMEKETTLE_RECORDING_ROOT").unwrap_or_else(|_| "landing/v1".to_owned());
        let prefix = timekettle_compactor::locate_landing_prefix(&cfg, recording_id, &root_prefix)?
            .ok_or_else(|| {
                format!(
    ...
    // Consumer shim: timekettle-tui / timekettle-semantic-metrics historically read the
...
fn resolve_recording_from_source(
    ...
    // Same consumer shim as the session-layout pull (timekettle-tui / metrics).
    let legacy_copy = root.root.join("recording").join("semantic-events.jsonl");
    if let Some(parent) = legacy_copy.parent() {
        let _ = std::fs::create_dir_all(parent);
    }
...
mod tests {
    ...
    fn register_test_schema_identity() {
        timekettle::register_table_identity([
    ...
    fn artifact_kind_constraints_cover_lifecycle_registrations() {
        ...
        let migrations = [
            include_str!("../../../timekettle-store/migrations/0001_init.sql"),
            include_str!("../../../timekettle-store/migrations/0002_artifact_kinds.sql"),
            include_str!("../../../timekettle-store/migrations/0003_session_manifests.sql"),
            include_str!("../../../timekettle-store/migrations/0004_call_ledger_artifact.sql"),
            include_str!("../../../timekettle-store/migrations/0005_seed_certificate_artifact.sql"),
            include_str!("../../../timekettle-store/migrations/0006_record_graph_artifact.sql"),
        ];
    ...
    fn certificate_seed_entry(boundary: &str, key: &str) -> timekettle::SeedEntry {
        timekettle::SeedEntry {
            boundary: boundary.to_owned(),
            key: key.to_owned(),
    ...
    fn seed_certificate_summarizes_materialized_skipped_failed_and_readback_states() {
        ...
        certificate.push(SeedCertificateEntry::new(
            ...
            &db,
            None,
            Some(timekettle::db_schema_for("cycle36b")),
            SeedMaterializationStatus::Skipped,
            SeedReadback::not_run("db seeding disabled by TIMEKETTLE_SEED_DB=0"),
        ));
        certificate.push(SeedCertificateEntry::new(
        ...
        certificate.push(SeedCertificateEntry::not_a_precondition(
            &Some("c1".to_owned()),
            "db",
            "timekettle:state:v1:db_row:7573657273:6964:31",
            timekettle::NotPreconditionReason::ReadAfterWrite,
        ));
    ...
    fn seed_certificate_preserves_db_row_and_query_seed_entries_when_db_seeding_is_skipped() {
        ...
        let query_key = timekettle::StateKey::DbQuery {
            table: "users".to_owned(),
            fingerprint: "find-user-by-email".to_owned(),
        }
        .to_wire();
        let row_key = timekettle::StateKey::DbRow {
            table: "users".to_owned(),
            key: vec![("user_id".to_owned(), user_id.to_owned())],
        }
        .to_wire();
        let query_result_image = timekettle::db::DbRowImage::new(
            "users",
            vec![
                timekettle::db::DbColumnImage {
                    wire: None,
                    name: "user_id".into(),
                    type_oid: Some(25),
        ...
        .to_value();
        let rmw_pre_image = timekettle::db::DbRowImage::new(
            "users",
            vec![
                timekettle::db::DbColumnImage {
                    wire: None,
        ...
        .to_value();
        let rmw_post_image = timekettle::db::DbRowImage::new(
            "users",
            vec![
                timekettle::db::DbColumnImage {
                    wire: None,
        ...
        let query_event: timekettle::BoundaryEvent = serde_json::from_value(serde_json::json!({
            "global_sequence": 50,
            "request_sequence": 1,
            ...
            "is_error": false,
            "duration_us": 0,
            "event_schema_version": timekettle::CURRENT_EVENT_SCHEMA_VERSION,
            "provenance": "recorded",
            "recon": "lossless",
        ...
        let rmw_event: timekettle::BoundaryEvent = serde_json::from_value(serde_json::json!({
            "global_sequence": 51,
            "request_sequence": 2,
            "correlation_id": corr.as_deref().unwrap(),
        ...
        let plan = timekettle::build_seed_plan(&[query_event, rmw_event], corr.as_deref());
        let mut entries = plan.iter().collect::<Vec<_>>();
        ...
        let schema = corr.as_deref().map(timekettle::db_schema_for);
        ...
        for entry in &certificate.entries {
            ...
            assert_eq!(entry.origin, timekettle::SeedOrigin::Recording);
    ...
    fn seed_certificate_db_readback_sql_separates_full_row_and_key_match_predicates() {
        ...
        let full_row_sql =
            build_count_sql(Some("timekettle_cycle36b"), &image, None).expect("full-row count SQL");
        assert!(full_row_sql.starts_with("SELECT COUNT(*) FROM \"timekettle_cycle36b\".\"users\""));
        ...
        let key_sql = build_count_sql(Some("timekettle_cycle36b"), &image, Some(&key_filter))
    ...
    fn isolated_parallel_replays_use_tail_ids_and_preserve_shared_opt_out() {
        ...
        let demo = Demo {
            ...
            compose_overlay: "compose.timekettle.yml".into(),
            project: "timekettle-demo-shared".into(),
            replay_port: shared_port,
            kernel_bin: "timekettle-kernel".into(),
            topic: "recording-events".into(),
            harness_state: "/tmp/timekettle-state".into(),
            candidate_image: None,
            ucs_profile: false,
        };
        ...
        assert_eq!(replay_a.project, "timekettle-run-00000001");
        assert_eq!(replay_b.project, "timekettle-run-00000002");
    ...
    fn record_graph_extract_keeps_only_graph_nodes_and_drops_payloads() {
        ...
        // Every emitted line is a GraphNode TimekettleRecord the /graph reader accepts.
        for line in out.lines() {
            assert!(matches!(
                serde_json::from_str::<timekettle::TimekettleRecord>(line),
                Ok(timekettle::TimekettleRecord::GraphNode(_))
            ));
        }
    }
        ...
        let run = Run {
            ...
            spec: RunSpec {
                ...
                candidate_spec: CandidateSpec::PrebuiltImage {
                    image: "timekettle-demo".to_owned(),
                },
                candidate_repo: None,
                recording_id: Some(recording_id.to_owned()),
                s3_source: None,
    ...
    fn a_failed_seed_is_named_in_the_scorecards_warnings() {
        ...
        let key = "timekettle:state:v1:db_row:7061796d656e745f617474656d7074:617474656d70745f6964:78";
    ...
    fn seed_plan_yields_settlement_rates_from_recording_and_template() {
        ...
        let mut plan = timekettle::SeedPlan::new();
        for entry in timekettle::build_seed_plan(&events, Some("c1")).iter() {
            plan.upsert(entry.clone());
        }
        let plan = plan.with_ambient(&timekettle::AmbientTemplate::demo_defaults());
        ...
        assert_eq!(default.origin, timekettle::SeedOrigin::Recording);
        ...
        assert_eq!(premium.origin, timekettle::SeedOrigin::Ambient);
        assert_eq!(
    ...
    fn ambient_template_defaults_to_demo_premium_rate() {
        // No TIMEKETTLE_AMBIENT_TEMPLATE set in test → demo defaults.
        let template = load_ambient_template();
        assert!(!template.is_empty());
        let plan = timekettle::SeedPlan::new().with_ambient(&template);
        assert_eq!(
            rendered_string(
                &plan
                    .resolve("redis", "settlement_rate_premium")
    ...
    #[test]
    fn redis_seed_decodes_timekettlerredisvalue_wrapper_to_raw_value() {
        // V1 regression: a recorded redis GET hit is an externally-tagged
        // TimekettleRedisValue, not a bare string. The seeder must write the DECODED
    ...
    fn db_query_seed_plan_materializes_users_ok_envelope_into_insert_sql() {
        ...
        let event: timekettle::BoundaryEvent = serde_json::from_value(serde_json::json!({
            "global_sequence": 0,
            "request_sequence": 0,
        ...
        let plan = timekettle::build_seed_plan(&[event], Some("cycle36b"));
        let seed = plan
            .resolve("db", &query_key)
            .expect("DbQuery read must seed from the recorded result envelope");
        assert_eq!(seed.origin, timekettle::SeedOrigin::Recording);
        ...
        let sql = build_insert_sql(Some("timekettle_cycle36b"), &rows[0]).expect("insert SQL");

        assert!(
            sql.starts_with("INSERT INTO \"timekettle_cycle36b\".\"users\""),
    ...
    fn db_row_seeds_materialize_before_query_fallback_for_same_payment_intent() {
        ...
        let mut plan = timekettle::SeedPlan::new();
        plan.upsert(timekettle::SeedEntry {
        ...
        let first_sql =
            build_insert_sql(Some("timekettle_confirm"), &first_rows[0]).expect("first insert sql");
        assert!(
        ...
        let second_sql =
            build_insert_sql(Some("timekettle_confirm"), &second_rows[0]).expect("second insert sql");
        assert!(
    ...
    fn seed_materialization_orders_by_source_sequence_before_kind() {
        let entry = |key: &str, seq: u64| timekettle::SeedEntry {
            boundary: "db".to_owned(),
            key: key.to_owned(),
            value: serde_json::json!({}),
            image: None,
    ...
    fn signin_users_tape_entry_materializes_dbquery_and_dbrow_seed_sql() {
        ...
        let users_row = serde_json::json!({
            ...
            "email": "user_8ab3599a75a5b997@timekettle.dev",
            "is_active": true,
            "is_verified": false,
        ...
        .unwrap();
        let plan = timekettle::build_seed_plan(&[event], Some(corr));
        let mut catalog = DbCatalog::default();
        catalog.insert(
            "users".into(),
            DbColumnMetadata {
        ...
        for key in [&query_key, &row_key] {
            ...
            let sql = build_insert_sql(Some(&timekettle::db_schema_for(corr)), &rows[0])
                .expect("signin users row must build INSERT SQL");
            assert!(sql.contains("\"totp_secret\"") && sql.contains("NULL"));
            assert!(sql.contains("'user_8ab3599a75a5b997@timekettle.dev'"));
    ...
    fn seed_db_renders_encrypted_bytea_key_as_hex_literal_from_metadata() {
        ...
        let sql = build_insert_sql(Some("timekettle_4d2c"), &image).expect("insert sql built");
        assert!(
            image
                .columns
                .iter()
    ...
    fn typed_db_image_metadata_is_preferred_and_all_unknown_image_falls_back() {
        ...
        let typed_sql = build_insert_sql(Some("timekettle_typed"), &rows[0]).expect("typed insert SQL");
        assert!(
        ...
        let all_unknown_image = timekettle::db::DbRowImage::new(
            "merchant_key_store",
            vec![
                timekettle::db::DbColumnImage {
                    wire: None,
        ...
        let unknown_sql =
            build_insert_sql(Some("timekettle_typed"), &unknown_rows[0]).expect("unknown insert SQL");
        assert!(
        ...
        let fallback_sql =
            build_insert_sql(Some("timekettle_typed"), &fallback_rows[0]).expect("fallback insert SQL");
        assert!(
    ...
    fn nullable_bytea_column_renders_null_instead_of_skipping_row() {
        ...
        let sql = build_insert_sql(Some("timekettle_signin"), &image)
        ...
        assert!(
            sql.contains("'user_8ab3599a75a5b997@timekettle.dev'"),
    ...
    fn create_schema_sql_clones_only_named_tables_not_all_public() {
        ...
        let sql =
            build_create_schema_sql("timekettle_corr_1", &["users".into(), "payment_intent".into()]);
        assert!(sql.contains("CREATE SCHEMA IF NOT EXISTS \"timekettle_corr_1\";"));
        assert!(
    ...
    fn create_schema_sql_empty_tables_is_bare_schema_only() {
        ...
        let sql = build_create_schema_sql("timekettle_empty", &[]);
        assert_eq!(sql, "CREATE SCHEMA IF NOT EXISTS \"timekettle_empty\"; ");
        assert!(!sql.contains("DO $timekettle$"));
    }
    ...
    fn wire_copy_out_command_renders_quoted_and_keyed() {
        ...
        assert_eq!(
            command,
            "\\copy (SELECT \"attempt_id\", \"connector_transaction_id\" FROM \"timekettle_c1\".\"payment_attempt\" \
    ...
    fn wire_copy_commands_stage_copy_and_land_with_conflict_tolerance() {
        ...
        assert_eq!(
            commands,
            vec![
                "CREATE TEMP TABLE _timekettle_wire_seed AS SELECT \"attempt_id\", \"connector_transaction_id\" FROM \"corr_1\".\"payment_attempt\" WITH NO DATA".to_string(),
                "\\copy _timekettle_wire_seed from pstdin with (format binary)".to_string(),
                "INSERT INTO \"corr_1\".\"payment_attempt\" (\"attempt_id\", \"connector_transaction_id\") SELECT \"attempt_id\", \"connector_transaction_id\" FROM _timekettle_wire_seed ON CONFLICT DO NOTHING".to_string(),
            ]
        );
    }
}

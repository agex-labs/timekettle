...
pub async fn apply_run_event(
    ...
    run_id: &str,
    ev: &RunEvent,
) -> Result<(), timekettle_store::StoreError> {
    match ev {
...
enum Inner {
    ...
    Http {
        ...
        base: String,
        /// Bearer token when the orchestrator has TIMEKETTLE_API_SERVICE_TOKEN set.
        token: Option<String>,
        /// X-Timekettle-Actor value (always required by the mutation boundary).
        actor: String,
        log_seq: Arc<AtomicI64>,
    },
}
        ...
        match &self.inner {
            ...
            Some(Inner::Http {
                ...
                for attempt in 1..=ATTEMPTS {
                    let mut req = agent.post(&url).set("x-timekettle-actor", actor);

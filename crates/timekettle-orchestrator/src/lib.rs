...
pub struct RunSpec {
    ...
    /// orchestrator substitutes it into `TIMEKETTLE_CANDIDATE_TARBALL_URL` (with the
    /// image's sha) to fetch that ref's `migrations/` and stage the P1 bundle
    /// (Option B). Unset → the orchestrator's `TIMEKETTLE_CANDIDATE_REPO` default.
    #[serde(default)]
...
/// orchestrator's environment (`TIMEKETTLE_S3_ACCESS_KEY` / `TIMEKETTLE_S3_SECRET_KEY`,
/// same as the session-layout path).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct S3Source {
    /// `s3://bucket/prefix` (scheme optional).
    pub path: String,
    /// AWS region; defaults to the orchestrator env's `TIMEKETTLE_S3_REGION`.
    #[serde(default)]
...
/// The most a run id may be. A k8s Job is named `timekettle-replay-{run_id}` and
...
/// each artifact from (for the Hyperswitch router: `ROUTER__TIMEKETTLE__REPLAY__SOURCE`
...
mod harness_root_tests {
    ...
    fn contract_paths_match_the_runners_own_write_paths() {
        let root = HarnessRoot::new(std::env::temp_dir().join("timekettle-test-contract"))
    ...
    fn contract_lookup_table_is_absolute_when_root_is() {
        let root = HarnessRoot::new(std::env::temp_dir().join("timekettle-test-contract-abs"))
            .expect("create harness root");
    ...
    fn wait_for_seed_snippet_targets_the_published_sentinel() {
        let root = HarnessRoot::new(std::env::temp_dir().join("timekettle-test-wait-seed"))
            .expect("create harness root");
...
mod run_params_tests {
    ...
    fn replay_spec() -> RunSpec {
        RunSpec {
            ...
            s3_source: Some(S3Source {
                path: "s3://timekettle/recordings/2026-08-05".to_owned(),
                region: None,
                endpoint: None,
            }),
    ...
    fn the_record_carries_the_whole_request() {
        ...
        assert_eq!(json["s3_source"]["path"], "s3://timekettle/recordings/2026-08-05");

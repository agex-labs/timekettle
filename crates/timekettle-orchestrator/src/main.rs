...
use timekettle_orchestrator::executor::{ExecutorKind, InClusterConfig, K8sExecutorConfig};
use timekettle_orchestrator::{api::runs, divergence, HarnessRoot, Run, RunStatus};
use timekettle_store::Store;
...
impl ExecutorSelection {
    /// Resolve from `TIMEKETTLE_EXECUTOR`. For k8s, the in-cluster config + Job
...
impl MutationAuth {
    fn from_env() -> Self {
        let service_token = std::env::var("TIMEKETTLE_API_SERVICE_TOKEN")
            .ok()
...
async fn main() {
    ...
    let root = match HarnessRoot::new(&root_dir) {
        Ok(r) => Arc::new(r),
        Err(err) => {
            eprintln!("timekettle-orchestrator: HARNESS_STATE_DIR setup failed: {err}");
            std::process::exit(1);
        }
    };
    ...
    // pg).
    let db_url =
        std::env::var("TIMEKETTLE_DB_URL").unwrap_or_else(|_| timekettle_store::DEFAULT_DB_URL.to_string());
    let store = match Store::connect(&db_url).await {
        Ok(s) => {
            eprintln!("timekettle-orchestrator: store connected + migrated ({db_url})");
            Some(Arc::new(s))
        }
        Err(err) => {
            eprintln!(
                "timekettle-orchestrator: store unavailable ({db_url}): {err} — running file-only; \
                 start it with: docker compose -p timekettle-orchestrator -f demo/docker-compose.orchestrator.yml up -d"
            );
            None
        }
    };
    let executor = match ExecutorSelection::from_env() {
        Ok(e) => {
            match &e {
                ExecutorSelection::Compose => eprintln!("timekettle-orchestrator: executor = compose"),
                ExecutorSelection::K8s(k) => eprintln!(
                    "timekettle-orchestrator: executor = k8s (jobs ns {}, template {}/{})",
        ...
        Err(err) => {
            eprintln!("timekettle-orchestrator: executor config failed: {err}");
            std::process::exit(1);
        }
    };
    let state = AppState {
    ...
    if let ExecutorSelection::K8s(k) = &*state.executor {
        match &state.store {
            Some(store) => timekettle_orchestrator::executor::reconcile::spawn(
                store.clone(),
                k.incluster.clone(),
                k.cfg.clone(),
            ),
            None => eprintln!(
                "timekettle-orchestrator: k8s reconciler disabled — no store (the reconciler needs \
    ...
    let listener = match tokio::net::TcpListener::bind(&bind_addr).await {
        Ok(l) => l,
        Err(err) => {
            eprintln!("timekettle-orchestrator: bind {bind_addr} failed: {err}");
            std::process::exit(1);
        }
    };
    eprintln!(
        "timekettle-orchestrator: listening on http://{bind_addr} (state: {})",
        root.root.display()
    );
    ...
    if let Err(err) = axum::serve(listener, app)
        ...
        .await
    {
        eprintln!("timekettle-orchestrator: server error: {err}");
        std::process::exit(1);
    }
}

fn app_router(state: AppState) -> Router {
    // Human create: audited via X-Timekettle-Actor, no service token (this endpoint is
...
async fn shutdown_signal() {
    ...
    let terminate = async {
        match tokio::signal::unix::signal(tokio::signal::unix::SignalKind::terminate()) {
            ...
            Err(e) => {
                eprintln!("timekettle-orchestrator: cannot install SIGTERM handler: {e}");
    ...
    eprintln!("timekettle-orchestrator: shutting down");
}
...
fn require_store(st: &AppState) -> Result<Arc<Store>, Response> {
    st.store.clone().ok_or_else(|| {
        error_resp(
            503,
            "store unavailable — start it: docker compose -p timekettle-orchestrator -f demo/docker-compose.orchestrator.yml up -d",
        )
    })
}

/// Human-facing mutations (`POST /runs`): identify the caller via `X-Timekettle-Actor`
...
async fn require_human_auth(mut req: Request<axum::body::Body>, next: Next) -> Response {
    let Some(actor) = actor_from_headers(req.headers()) else {
        return error_resp(401, "X-Timekettle-Actor header required for mutating requests");
    };
...
/// authenticates with the shared `TIMEKETTLE_API_SERVICE_TOKEN` (plus `X-Timekettle-Actor`
...
/// `AuthenticatedActor` from `X-Timekettle-Actor` (the audit identity). This is a
...
async fn v1_create_run(
    ...
    let expectation = raw
        ...
        .and_then(|v| v.as_str())
        .map(str::to_owned);
    let spec: timekettle_orchestrator::RunSpec = match serde_json::from_value(raw) {
        Ok(s) => s,
        Err(e) => return error_resp(400, &format!("parse RunSpec: {e}")),
    };
    ...
    if let Err(e) =
        timekettle_orchestrator::scope::check_requested_correlations(spec.correlation_filter.as_deref())
    {
    ...
    let ctx = if let Some(store) = &st.store {
        ...
        let params =
            timekettle_orchestrator::RunParams::resolved(&run.spec, expectation.as_deref()).to_json();
        if let Err(e) = store
            .insert_run(
                &run.run_id,
            ...
            .await
        {
            eprintln!("timekettle-orchestrator: store insert_run failed: {e}");
        }
        let _ = store
            .audit(
                &actor,
            ...
            .await;
        timekettle_orchestrator::lifecycle::StoreCtx::new(
            &run.run_id,
            Some((tokio::runtime::Handle::current(), store.clone())),
        )
    } else {
        timekettle_orchestrator::lifecycle::StoreCtx::disabled(&run.run_id)
    };
    match &*st.executor {
...
async fn v1_kill_run(
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let transport = timekettle_orchestrator::executor::UreqTransport::new(&incluster)
            .map_err(|e| format!("k8s client: {e}"))?;
        let api = timekettle_orchestrator::executor::KubeApi::new(transport);
        let report = timekettle_orchestrator::executor::kill_run(&api, &namespace, &id)
        ...
        let ctx = match &store {
            Some(s) => timekettle_orchestrator::lifecycle::StoreCtx::new(
                &id,
                Some((handle, std::sync::Arc::clone(s))),
            ),
            None => timekettle_orchestrator::lifecycle::StoreCtx::disabled(&id),
        };
...
async fn v1_ingest_run_event(
    ...
    use timekettle_orchestrator::lifecycle::store_ctx::{apply_run_event, RunEvent};
    ...
    let mut run: Run = match timekettle_orchestrator::read_json(&run_path) {
        Ok(run) => run,
    ...
    if matches!(run.status, RunStatus::Completed | RunStatus::Failed) {
        match &ev {
            RunEvent::Stage { .. } | RunEvent::State { .. } => {
                eprintln!(
                    "timekettle-orchestrator: dropping post-terminal progress event for {run_id} \
                     (settled {:?})",
                    run.status
                );
            ...
            RunEvent::Finish { ok, .. } => {
                ...
                if incoming != run.status {
                    eprintln!(
                        "timekettle-orchestrator: conflicting finish for {run_id}: keeping settled \
                         {:?}, ignoring {incoming:?}",
                        run.status
                    );
                }
    ...
    let file_side_changed = match &ev {
        RunEvent::Stage { stage, step, total } => {
            ...
            run.stage_updated_ms = timekettle_orchestrator::now_ms();
            true
        }
    ...
    if file_side_changed {
        if let Err(e) = timekettle_orchestrator::write_json(&run_path, &run) {
    ...
    if let Some(store) = &st.store {
        if let Err(e) = apply_run_event(store, &run_id, &ev).await {
            eprintln!("timekettle-orchestrator: run-event store write failed for {run_id}: {e}");
        }
    }
...
/// partitioned, belong to the deployment (`TIMEKETTLE_S3_BUCKET`,
/// `TIMEKETTLE_RECORDING_ROOT`) — a caller names a recording and the orchestrator
/// resolves the rest.
///
...
async fn v1_available_recordings(
    ...
    let cfg = timekettle_orchestrator::s3::S3Config::from_env();
    let root = std::env::var("TIMEKETTLE_RECORDING_ROOT").unwrap_or_else(|_| "landing/v1".to_owned());
    let found = match tokio::task::spawn_blocking(move || {
        timekettle_compactor::list_landed_recordings(&cfg, &root)
    })
    .await
    {
        Ok(Ok(found)) => found,
    ...
    let page: Vec<serde_json::Value> = found
        ...
        .map(|r| {
            ...
            let identity = timekettle_orchestrator::parse_recording_id(&r.session_id);
            let described = match &identity {
                timekettle_orchestrator::RecordingIdentity::Described {
                    revision,
                    recorded_at,
                    instance,
                } => serde_json::json!({
                ...
                timekettle_orchestrator::RecordingIdentity::Opaque => serde_json::Value::Null,
            };
            serde_json::json!({
...
enum RecordingCorrelations {
    Sealed(Vec<timekettle_orchestrator::s3::CorrelationSummary>),
...
/// correlations drives the first [`timekettle_orchestrator::scope::MAX_CORRELATIONS_PER_RUN`]
...
async fn v1_recording_correlations(
    ...
    let found = match tokio::task::spawn_blocking(move || -> Result<_, String> {
        match timekettle_orchestrator::s3::read_correlation_index(&cfg, &wanted)? {
            ...
            None => Ok(
                match timekettle_compactor::locate_landing_prefix(&cfg, &wanted, &scanned)? {
    ...
    match found {
        RecordingCorrelations::Sealed(rows) => {
            ...
            let cases: Vec<&timekettle_orchestrator::s3::CorrelationSummary> = rows
                .iter()
                .filter(|row| row.correlation_id.is_some())
                .collect();
            let matched: Vec<&&timekettle_orchestrator::s3::CorrelationSummary> = cases
                .iter()
                ...
                .collect();
            let page: Vec<&timekettle_orchestrator::s3::CorrelationSummary> = matched
                .iter()
                .skip(offset)
                .take(limit)
                .map(|row| **row)
            ...
            json_ok(serde_json::json!({
                ...
                "total": cases.len(),
                "matched": matched.len(),
                "max_per_run": timekettle_orchestrator::scope::MAX_CORRELATIONS_PER_RUN,
                "offset": offset,
                "limit": limit,
                "correlations": page,
            }))
...
async fn hydrate_run_artifacts(st: &AppState, run_id: &str) {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        for art in arts {
            ...
            let Ok((bucket, key)) = timekettle_orchestrator::codebundle::parse_s3_uri(&art.uri) else {
                continue; // not an s3:// uri (compose local path) — nothing to pull
            };
            let mut cfg = timekettle_orchestrator::s3::S3Config::from_env();
            cfg.bucket = bucket;
            match timekettle_compactor::get_object_decoded(&cfg, &key) {
                Ok(bytes) => {
...
/// shared `TimekettleRecord` stream: record-side in the recording tape, replay-side
...
async fn v1_graph(State(st): State<AppState>, Path(id): Path<String>) -> Response {
    ...
    let read_nodes = |path: std::path::PathBuf| -> Vec<serde_json::Value> {
        ...
        std::io::BufRead::lines(std::io::BufReader::new(file))
            ...
            .filter_map(
                |line| match serde_json::from_str::<timekettle::TimekettleRecord>(&line) {
                    Ok(timekettle::TimekettleRecord::GraphNode(node)) => serde_json::to_value(node).ok(),
                    Ok(timekettle::TimekettleRecord::BoundaryEvent(_) | timekettle::TimekettleRecord::Observed(_)) => None,
                    Err(_) => None,
                },
            )
            .collect()
    ...
    if record.is_empty() && record_note.is_none() {
        if let Ok(run) = runs::get(&st.root, &id) {
            let scope = timekettle_orchestrator::scope::RunScope::of(&run);
            if let Some(rec) = run.recording_id.clone().or(run.spec.recording_id.clone()) {
                match timekettle_orchestrator::scope::ScopedRecording::open(&st.root, &rec, scope) {
...
async fn v1_artifact_raw(State(st): State<AppState>, Path(id): Path<i64>) -> Response {
    ...
    let bytes = if let Ok((bucket, key)) = timekettle_orchestrator::codebundle::parse_s3_uri(&art.uri) {
        let fetch = tokio::task::spawn_blocking(move || {
            let mut cfg = timekettle_orchestrator::s3::S3Config::from_env();
            cfg.bucket = bucket;
            timekettle_compactor::get_object_decoded(&cfg, &key)
        })
        .await;
        match fetch {
            Ok(Ok(b)) => b,
...
mod tests {
    ...
    // Human mutation boundary (POST /runs): X-Timekettle-Actor only, no service token.
    ...
    // Service callback boundary (POST /runs/{id}/events): X-Timekettle-Actor plus the
    // bearer token when TIMEKETTLE_API_SERVICE_TOKEN is configured.
    ...
    async fn oneshot_status(
        ...
        if let Some(actor) = actor {
            builder = builder.header("X-Timekettle-Actor", actor);
        }
        router
    ...
    fn pending_run(run_id: &str) -> Run {
        Run {
            ...
            spec: timekettle_orchestrator::RunSpec {
                mode: timekettle_orchestrator::RunMode::Replay,
                candidate_spec: timekettle_orchestrator::CandidateSpec::PrebuiltImage {
    ...
    async fn post_event(state: AppState, run_id: &str, body: serde_json::Value) -> StatusCode {
        let req = Request::builder()
            ...
            .header("X-Timekettle-Actor", "system:test-runner")
    ...
    async fn ingest_mirrors_stage_and_finish_into_the_run_record() {
        ...
        timekettle_orchestrator::write_json(&state.root.run_path("run-ev"), &run).unwrap();
        ...
        let run: Run = timekettle_orchestrator::read_json(&state.root.run_path("run-ev")).unwrap();
    ...
    async fn ingest_terminal_guard_ignores_post_finish_events() {
        let dir = tempfile::tempdir().unwrap();
        let state = test_state(dir.path());
        timekettle_orchestrator::write_json(&state.root.run_path("run-t"), &pending_run("run-t"))
            .unwrap();
        ...
        let run: Run = timekettle_orchestrator::read_json(&state.root.run_path("run-t")).unwrap();
        assert!(
    ...
    async fn ingest_rejects_unknown_run_and_unknown_state() {
        ...
        timekettle_orchestrator::write_json(&state.root.run_path("run-ev2"), &pending_run("run-ev2"))
            .unwrap();
        let status = post_event(
            state.clone(),
            "run-ev2",

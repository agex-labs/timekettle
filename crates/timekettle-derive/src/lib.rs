...
pub fn recordable(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // `#[timekettle_derive::recordable(local)]` — no #[macro_export], for same-crate use
...
///     result = response_result(__timekettle_result),
/// )]
/// async fn send_request(...) -> Result<Response, Error> { ... }
/// ```
///
/// `correlation` must evaluate to `Option<String>`. `args` must evaluate to
/// `serde_json::Value`. `result` receives `__timekettle_result` as `&Output` and must
...
/// - args captured per-argument via `timekettle::capture!` (structured serde when the

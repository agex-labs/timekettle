...
#[derive(Clone, Copy)]
pub enum Preset {
    /// No declared defaults (`#[timekettle::instrument]`, ...).
    None,
    /// `timekettle::redis` ⇒ `Execute`, `kind = "redis"`. The default for redis reads +
    ...
    /// never be Execute.)
    Redis,
    /// `timekettle::id` ⇒ `Substitute`, `kind = "id"` (entropy is reconstructed, never re-run).
    Id,
    /// `timekettle::time` ⇒ `Substitute`, `kind = "time"`.
    Time,
    /// `timekettle::http(outgoing)` ⇒ `Substitute`, `kind = "http"` (egress never re-issued).
    HttpOutgoing,
    /// `timekettle::http(incoming)` ⇒ declares NOTHING. Ingress is the replay DRIVER
...
pub fn generate_http(attr: proc_macro::TokenStream, item: proc_macro::TokenStream) -> TokenStream {
    ...
    // `timekettle::http(incoming)` is the replay DRIVER (correlation seed): it still
...
fn generate_inner(args: InstrumentArgs, mut func: ItemFn, preset: Preset) -> TokenStream {
    ...
    //
    // We DELIBERATELY do NOT fold the signature into the hash. Timekettle's purpose is
    ...
    // hashes `boundary::component::operation` with no signature (timekettle/src/lib.rs).
    //
    ...
    let identity_build: TokenStream = quote! {
        let __timekettle_identity_scope: ::std::string::String = { #identity_scope_expr };
        let __timekettle_identity_correlation: ::std::option::Option<::std::string::String> =
            match &__timekettle_boundary_correlation_id {
                ::std::option::Option::Some(c) => ::std::option::Option::Some(c.clone()),
                ::std::option::Option::None => ::timekettle::__private::current_correlation_id(),
            };
        let __timekettle_identity = ::timekettle::__private::CallsiteIdentity {
            version: 1,
            source: ::timekettle::__private::CallsiteSource::SyntacticHash,
            id: ::std::option::Option::None,
            scope: ::std::option::Option::Some(__timekettle_identity_scope.clone()),
            occurrence: ::timekettle::__private::next_boundary_occurrence(
                __timekettle_identity_correlation.as_deref(),
                ::timekettle::__private::CallsiteSource::SyntacticHash,
                ::std::option::Option::Some(__timekettle_identity_scope.as_str()),
            ),
            ...
            span_path: ::timekettle::__private::current_span_path(),
        };
    };
    ...
    // path is a custom `::timekettle::codec::ReplayCodec` impl.
    ...
    let result_expr: Expr = args.result.clone().unwrap_or_else(|| match &capture_mode {
        CaptureMode::Custom(path) => {
            parse_quote!(<#path as ::timekettle::codec::ReplayCodec>::capture(__timekettle_result))
        }
        CaptureMode::Serde => parse_quote!(::timekettle::value::result_serialize(__timekettle_result)),
        CaptureMode::ResultOk => parse_quote!(::timekettle::value::result_serialize_ok(__timekettle_result)),
        CaptureMode::Debug => {
            parse_quote!(::timekettle::value::result_debug(__timekettle_result))
        }
    });
    ...
    let reconstruct_closure: TokenStream = match &capture_mode {
        CaptureMode::Custom(path) => quote! {
            |__timekettle_recorded: ::serde_json::Value| -> ::timekettle::__private::Reconstructed<#recon_ty> {
                match <#path as ::timekettle::codec::ReplayCodec>::reconstruct(__timekettle_recorded) {
                    ::std::option::Option::Some(__timekettle_replayed) =>
                        ::timekettle::__private::Reconstructed::Value(__timekettle_replayed),
                    ::std::option::Option::None => ::timekettle::__private::Reconstructed::Failed(
                        ::std::format!(
        ...
        CaptureMode::ResultOk => {
            ...
            quote! {
                |__timekettle_recorded: ::serde_json::Value| -> ::timekettle::__private::Reconstructed<#recon_ty> {
                    if __timekettle_recorded
                        .as_object()
                        .is_some_and(|__timekettle_map| __timekettle_map.contains_key("timekettle_err"))
                    {
                        return ::timekettle::__private::Reconstructed::Failed(
                            ::std::string::String::from(
                                "recorded payload is a captured error (`timekettle_err`), not an Ok value"
                            )
                        );
                    }
                    match ::serde_json::from_value::<#ok_ty>(__timekettle_recorded) {
                        ::std::result::Result::Ok(__timekettle_replayed) =>
                            ::timekettle::__private::Reconstructed::Value(::std::result::Result::Ok(__timekettle_replayed)),
                        ::std::result::Result::Err(__timekettle_err) => ::timekettle::__private::Reconstructed::Failed(
                            ::std::format!(
                                "cannot deserialize the recorded value into `{}`: {}",
                                ::std::stringify!(#ok_ty), __timekettle_err
                            )
                        ),
                    }
                }
        ...
        CaptureMode::Serde => quote! {
            |__timekettle_recorded: ::serde_json::Value| -> ::timekettle::__private::Reconstructed<#recon_ty> {
                match ::serde_json::from_value::<#recon_ty>(__timekettle_recorded) {
                    ::std::result::Result::Ok(__timekettle_replayed) =>
                        ::timekettle::__private::Reconstructed::Value(__timekettle_replayed),
                    ::std::result::Result::Err(__timekettle_err) => ::timekettle::__private::Reconstructed::Failed(
                        ::std::format!(
                            "cannot deserialize the recorded value into `{}`: {}",
                            ::std::stringify!(#recon_ty), __timekettle_err
                        )
                    ),
                }
            }
        ...
        CaptureMode::Debug => quote! {
            |_: ::serde_json::Value| -> ::timekettle::__private::Reconstructed<#recon_ty> {
                ::timekettle::__private::Reconstructed::Failed(
                    ::std::format!(
    ...
    let firewalled_prep = quote! {
        ::std::panic::catch_unwind(::std::panic::AssertUnwindSafe(|| {
            let __timekettle_boundary_correlation_id: ::std::option::Option<String> = { #correlation_expr };
            #identity_build
            ...
            let __timekettle_boundary_args: ::serde_json::Value = #args_expr;
            let __timekettle_observation = #crossing_observation_expr;
            (__timekettle_observation, __timekettle_boundary_args)
        }))
    };
    ...
    if sig.asyncness.is_some() {
        ...
        quote! {
            ...
            #vis #sig {
                ...
                // inert as Disabled.
                if ::timekettle::__private::observation_is_active() {
                    ...
                    let __timekettle_caller = ::std::panic::Location::caller();
                    match #firewalled_prep {
                        ::std::result::Result::Ok((__timekettle_observation, __timekettle_boundary_args)) => {
                            ::timekettle::__private::dispatch_async(
                                __timekettle_observation,
                                move || __timekettle_boundary_args,
                                move || async move #block,
                                #reconstruct_closure,
                                move |__timekettle_result| { #result_expr },
                            ).await
                        }
        ...
        quote! {
            ...
            #vis #sig {
                ...
                if ::timekettle::__private::observation_is_active() {
                    ...
                    match #firewalled_prep {
                        ::std::result::Result::Ok((__timekettle_observation, __timekettle_boundary_args)) => {
                            ::std::boxed::Box::pin(::timekettle::__private::dispatch_async(
                                __timekettle_observation,
                                move || __timekettle_boundary_args,
...
fn build_crossing_observation_expr(
    ...
    //
    // Uses the pre-bound `__timekettle_caller` rather than `Location::caller()` directly:
    ...
    // call site. The shape binds `__timekettle_caller` in the `#[track_caller]` fn body
    // (outside the closure) and the closure captures it.
    let mut observation = quote! {
        ::timekettle::__private::CrossingObservation::with_correlation(
            #boundary_spec_expr,
            __timekettle_identity,
            __timekettle_caller,
            __timekettle_boundary_correlation_id,
        )
    };
...
/// (#28). Combines the PRESET default (`timekettle::id`/`time`/`http` → `Substitute` +
...
fn build_boundary_spec_expr(
    ...
    let preset_effect: Option<TokenStream> = match preset {
        Preset::Redis => Some(quote!(::timekettle::__private::EffectKind::Redis)),
        Preset::Id => Some(quote!(::timekettle::__private::EffectKind::Entropy)),
        Preset::Time => Some(quote!(::timekettle::__private::EffectKind::Time)),
        Preset::HttpOutgoing => Some(quote!(::timekettle::__private::EffectKind::Http)),
        Preset::None | Preset::HttpIncoming => None,
    };
    ...
    let declaration_field = if has_declaration {
        quote! {
            ::std::option::Option::Some(::timekettle::__private::BoundaryDeclaration {
                effect: #effect_field,
                op: #op_field,
    ...
    // The preset's DEFAULT replay routing: `timekettle::redis` re-executes reads +
    ...
    let preset_default_strategy = match preset {
        Preset::Redis => quote!(::timekettle::__private::ReplayStrategy::Execute),
        _ => quote!(::timekettle::__private::ReplayStrategy::Substitute),
    };
    ...
    if !declared {
        ...
        return Ok(quote! {
            ::timekettle::__private::BoundarySpec::new(#boundary, #component, #operation)
        });
    }
    ...
    Ok(quote! {
        ::timekettle::__private::BoundarySpec::with_semantics(
            #boundary,
            #component,
            #operation,
            ::timekettle::__private::BoundarySemantics {
...
fn replay_strategy_variant(id: &Ident) -> std::result::Result<TokenStream, TokenStream> {
    match id.to_string().as_str() {
        "Execute" | "Substitute" => Ok(quote!(::timekettle::__private::ReplayStrategy::#id)),
        _ => Err(
...
fn effect_kind_variant(id: &Ident) -> std::result::Result<TokenStream, TokenStream> {
    match id.to_string().as_str() {
        "Db" | "Redis" | "Imc" | "Http" | "Grpc" | "Entropy" | "Time" | "Function" => {
            Ok(quote!(::timekettle::__private::EffectKind::#id))
        }
...
fn operation_kind_variant(id: &Ident) -> std::result::Result<TokenStream, TokenStream> {
    match id.to_string().as_str() {
        ...
        | "CompareAndSet" | "IdempotentDelete" | "ExternalCall" | "Entropy" | "Clock" => {
            Ok(quote!(::timekettle::__private::OperationKind::#id))
        }
        _ => Err(syn::Error::new_spanned(
            id,
...
fn return_semantics_variant(id: &Ident) -> std::result::Result<TokenStream, TokenStream> {
    match id.to_string().as_str() {
        ...
        | "PostImage" | "UpdateReturning" | "DeleteReturning" | "Raw" => {
            Ok(quote!(::timekettle::__private::ReturnSemantics::#id))
        }
        _ => Err(syn::Error::new_spanned(
            id,
...
enum CaptureMode {
    ...
    ResultOk,
    /// A custom `::timekettle::codec::ReplayCodec` impl (`codec = some::Codec`).
    Custom(Path),
...
fn inferred_args_expr(
    ...
    if !skip_all {
        for input in &sig.inputs {
            ...
            inserts.push(quote! {
                __timekettle_boundary_map.insert(
                    #key.to_string(),
                    ::timekettle::capture!(#ident),
                );
            });
        }
    }
    ...
    for field in fields {
        ...
        inserts.push(quote! {
            __timekettle_boundary_map.insert(
                #key.to_string(),
                ::timekettle::capture!((#expr)),
            );
        });
    }

    parse_quote!({
        let mut __timekettle_boundary_map = ::serde_json::Map::new();
        #(#inserts)*
        ::serde_json::Value::Object(__timekettle_boundary_map)
    })
}
...
///
/// MUST stay byte-for-byte identical to `timekettle_runtime::stable_callsite_hash`
...
pub(crate) fn syntactic_hash(input: &str) -> u64 {
    ...
    // Terminator byte, matching `fnv1a_str` in timekettle-record.
...
pub struct InstrumentArgs {
    ...
    /// custom `::timekettle::codec::ReplayCodec` impl whose `Value` must equal the
    /// return type.
    ...
    /// maps it to a `timekettle::__private::ReplayStrategy` value — the wire field on
                ...
                _ => {
                    if !input.peek(Token![=]) {
                        return Err(syn::Error::new(
                            key.span(),
                            "unsupported timekettle instrument argument",
                        ));
                    }
                    input.parse::<Token![=]>()?;
                    match key_string.as_str() {
...
mod tests {
    ...
    fn codec_selector_is_canonical_and_removed_aliases_name_the_replacement() {
        ...
        for (name, legacy) in [
            ...
            assert!(
                err.to_string()
                    .contains("unsupported timekettle instrument argument"),
                "legacy `{name}` must reject through the unsupported-argument path; got: {err}"
            );
        }
    }

    /// PRESETS after #28: `timekettle::id`/`time`/`http(outgoing)` default to
    /// `ReplayStrategy::Substitute` and carry a descriptive `kind` label; they emit
    /// no Channel/Effect taxonomy. `timekettle::http(incoming)` declares NOTHING (the
    ...
    fn state_capture_chaining_is_shared_by_all_generated_shapes() {
        ...
        for (src, future) in cases {
            ...
            let args_pos = expanded
                .find("__timekettle_boundary_args")

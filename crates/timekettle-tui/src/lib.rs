...
use timekettle_runtime::{BoundaryEvent, TimekettleRecord};
...
fn read_tape(path: &Path) -> Result<TapeLoad> {
    ...
    for line in reader.lines() {
        ...
        match serde_json::from_str::<TimekettleRecord>(&line) {
            Ok(TimekettleRecord::BoundaryEvent(event)) => events.push(*event),
            Ok(TimekettleRecord::GraphNode(node)) => graph_records.push(*node),
            Ok(TimekettleRecord::Observed(_)) | Err(_) => {
...
fn parse_semantic_event(line: &str) -> Option<BoundaryEvent> {
    ...
    serde_json::from_value::<TimekettleRecord>(value.clone())
        .ok()
        .and_then(|record| match record {
            TimekettleRecord::BoundaryEvent(event) => Some(*event),
            _ => None,
        })
...
fn parse_graph_record(line: &str) -> Option<ExecutionGraphNode> {
    ...
    serde_json::from_value::<TimekettleRecord>(value.clone())
        .ok()
        .and_then(|record| match record {
            TimekettleRecord::GraphNode(node) => Some(*node),
            _ => None,
        })
...
mod tests {
    ...
    fn loads_nested_artifacts_and_counts_skipped_lines() {
        ...
        // One tagged TimekettleRecord stream: a boundary event, two graph nodes, and
    ...
    fn temp_artifact_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "timekettle-tui-test-{label}-{}-{nanos}",
            std::process::id()
        ))
    }
}

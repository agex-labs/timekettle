...
use timekettle_runtime::BoundaryEvent;
use timekettle_tui::{
...
const USAGE: &str = "USAGE:
    timekettle-tui [--summary] <ARTIFACT_PATH>
...
EXAMPLES:
    cargo run -p timekettle-tui -- demo/harness-state
    cargo run -p timekettle-tui -- --summary demo/harness-state/<run-tag>
";
...
fn render_header(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let line = Line::from(vec![
        Span::styled(
            "Timekettle/Hyperswitch Artifacts",
...
fn render_footer(frame: &mut Frame<'_>, area: Rect, app: &App) {
    let mode = match app.input_mode {
        ...
        InputMode::Request => {
            "request: regex supported (e.g. timekettle-.+) | enter commit | esc cancel".to_owned()
        }
    };
    frame.render_widget(

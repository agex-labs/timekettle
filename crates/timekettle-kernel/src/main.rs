...
fn main() -> ExitCode {
    if let Err(err) = run() {
        eprintln!("timekettle-kernel: {err}");
...
fn run() -> Result<(), String> {
    ...
    eprintln!(
        "timekettle-kernel: loaded {} events from {recording_path}",
        events.len()
    );

    let (by_corr, uncorrelated) = group_by_correlation(events);
    eprintln!(
        "timekettle-kernel: {} correlations, {} uncorrelated background events",
        by_corr.len(),
        uncorrelated.len()
    );
    ...
    // can pass while those generators are migrated onto timekettle boundaries.
    ...
    if allowlist.is_empty() {
        eprintln!("timekettle-kernel: body allowlist empty (byte-exact mode)");
    } else {
        eprintln!(
            "timekettle-kernel: body allowlist ({}): {}",
            allowlist.len(),
            allowlist.join(", ")
        );
    }
    ...
    if let Some(filter) = parse_correlation_filter(std::env::var("KERNEL_CORRELATION_FILTER").ok())
    {
        ...
        eprintln!(
            "timekettle-kernel: correlation filter ({} id(s)): driving {} of {before} correlations ({} filtered out)",
            filter.len(),
            ordered.len(),
            before - ordered.len(),
        );
        for want in &filter {
            if !by_corr.contains_key(want) {
                eprintln!("timekettle-kernel: WARNING filter correlation {want} is not in the recording");
            }
        }
    }
    ...
    let drive_one = |cid: &String, events: &Vec<BoundaryEvent>| {
        match reconstruct_driver_request(events) {
            ...
            Some(mut driver) => {
                ...
                eprintln!(
                    "timekettle-kernel: drove {cid} → {}{} (status {} vs {}, body diffs {}{})",
                    driver.method,
                    driver.path,
    ...
    if concurrency <= 1 {
        ...
        eprintln!(
            "timekettle-kernel: CONCURRENT DRIVE — {} correlations across {} threads (KERNEL_DRIVE_CONCURRENCY={concurrency})",
            ordered.len(),
            concurrency,
        );
    ...
    eprintln!("timekettle-kernel: complete (driven {driven}, skipped {skipped})");
    Ok(())
}
...
fn load_recording(path: &PathBuf) -> std::io::Result<Vec<BoundaryEvent>> {
    ...
    for line in reader.lines() {
        ...
        let value = match serde_json::from_str::<serde_json::Value>(&line) {
            Ok(value) => value,
            Err(err) => {
                eprintln!("timekettle-kernel: skipping unparseable line: {err}");
                continue;
            }
        };
        match value.get("record_kind").and_then(|k| k.as_str()) {
            Some("boundary_event") => match serde_json::from_value::<BoundaryEvent>(value) {
                ...
                Err(err) => {
                    eprintln!("timekettle-kernel: skipping unparseable boundary_event: {err}");
                }
            },
            Some("graph_node") => graph_nodes += 1,
            other => {
                eprintln!("timekettle-kernel: skipping line with record_kind {other:?}");
            }
        }
    }
    if graph_nodes > 0 {
        eprintln!("timekettle-kernel: skipped {graph_nodes} graph_node record(s) riding the tape");
    }
    Ok(events)
}
...
fn drive(
    ...
    match drive_inner(target_host, target_port, driver) {
        ...
        Err(err) => {
            ...
            // response at all.
            eprintln!(
                "timekettle-kernel: NO RESPONSE from candidate for {} {}{} — {err}",
...
/// connection; replaying them against a body timekettle re-materialized
...
mod tests {
    ...
    fn driver(headers: Vec<(&str, &str)>, body: Option<&[u8]>) -> DriverRequest {
        use timekettle_kernel::BaselineResponse;
        DriverRequest {
    ...
    /// chunked` over timekettle's unchunked body desynchronizes the candidate's

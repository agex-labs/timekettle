...
pub use timekettle::BoundaryEvent;
...
pub fn reconstruct_driver_request(events: &[BoundaryEvent]) -> Option<DriverRequest> {
    ...
    // The recorder stores the request body under `request_body` (timekettle's
...
/// multimap object `{ "accept": ["*/*"], "host": ["h"] }` (timekettle::http::headers);
...
mod tests {
    ...
    fn reconstruct_handles_real_recorder_shape() {
        // The shape timekettle ACTUALLY produces (verified against a real recording):
    ...
    /// Byte-for-byte what `timekettle-runtime`'s `inject_body_json` writes for a

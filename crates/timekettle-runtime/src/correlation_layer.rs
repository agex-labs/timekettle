...
//! timekettle-context, so boundary events fired from spawned tasks inherit the request
...
//! differing only in that it is entered into timekettle-context on a CHANGE (≈once per
...
/// `timekettle.fork = true` (minted by [`crate::fork_span`] at the `tokio::spawn` site)
...
const FORK_FIELD: &str = "timekettle.fork";
...
thread_local! {
    ...
    /// The correlation this layer has entered into timekettle-context on this thread,
...
/// Enter `target` into timekettle-context only when it differs from what this layer last
...
fn engage_correlation(span_id: u64, target: Option<Arc<str>>) {
    CURRENT_CORRELATION.with(|current| {
        ...
        timekettle_context::set_current_correlation(target.as_deref());
        *current = target;
    });
}
...
fn restore_correlation(span_id: u64) {
    ...
    if let Some((_, previous)) = restored {
        timekettle_context::set_current_correlation(previous.as_deref());
...
/// Tracing layer mirroring the ingress `request_id` span field into timekettle-context.
#[derive(Debug, Default)]
pub struct TimekettleCorrelationLayer;

impl TimekettleCorrelationLayer {
...
/// Visitor that detects the `timekettle.fork` boundary marker on a span.
...
impl<S> Layer<S> for TimekettleCorrelationLayer
where
    S: Subscriber,
    S: for<'lookup> LookupSpan<'lookup>,
{
    fn on_new_span(&self, attrs: &Attributes<'_>, id: &Id, ctx: Context<'_, S>) {
        ...
        // A `timekettle.fork` span opens a fresh, unordered lineage bucket; every other
        ...
        let observe = if own_correlation.is_some() {
            correlation.as_deref().is_some_and(|id| {
                !matches!(
                    timekettle_context::recording_decision(id),
                    Some(timekettle_context::RecordDecision::Skip)
                )
            })
        } else {
...
mod tests {
    use super::*;
    use timekettle_context::current_correlation_id;
    use tracing_subscriber::prelude::*;

    #[test]
    fn enters_and_restores_correlation_around_a_span() {
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            assert_eq!(current_correlation_id(), None);
            let span = tracing::info_span!("timekettle::http_incoming", request_id = "req-42");
            {
    ...
    fn child_span_inherits_root_correlation() {
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-7");
    ...
    fn nested_spans_restore_lifo() {
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let outer = tracing::info_span!("timekettle::http_incoming", request_id = "outer");
    ...
    fn skip_decision_leaves_correlation_disengaged() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            let span =
                tracing::info_span!("timekettle::http_incoming", request_id = "req-skip-disengage");
            {
                let _entered = span.enter();
                assert_eq!(
    ...
    fn spawned_child_entered_without_parent_reverts_every_cursor() {
        ...
        // address or bucket.
        //
        // The path assertion used to run the other way — `Some("timekettle::http_incoming")`
        ...
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-spawn");
            let child = {
                let _root = root.enter();
            ...
            assert!(current_span_lineage().is_none());
            {
                ...
                assert_eq!(
                    current_span_path().as_deref(),
                    Some("timekettle::http_incoming>spawned_work")
                );
                assert!(
    ...
    fn fork_span_opens_a_new_lineage_bucket() {
        // The substrate's lineage proof: a `timekettle.fork`-marked span (what
        ...
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-fork");
            let _root = root.enter();
    ...
    fn an_entered_span_still_resolves_its_own_path_and_lineage() {
        ...
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-live");
            let detached = {
                let _root = root.enter();
                ...
                assert_eq!(
                    path.as_deref(),
                    Some("timekettle::http_incoming>update_trackers"),
    ...
    fn a_thread_that_served_a_request_resolves_neither_afterwards() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            let spawned = {
                let first = tracing::info_span!("timekettle::http_incoming", request_id = "req-earlier");
            ...
            };
            {
                ...
                assert_eq!(
                    current_span_path().as_deref(),
                    Some("timekettle::http_incoming>payments_core"),
    ...
    fn a_finished_fork_leaves_no_bucket_for_the_next_boundary_to_borrow() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            let (inside_fork, forked_bucket) = {
                let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-borrow");
                let _root = root.enter();
                let fork = crate::fork_span();
                let _fork = fork.enter();
    ...
    fn a_balanced_enter_and_exit_leaves_the_cursors_as_it_found_them() {
        ...
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-balance");
    ...
    fn a_guard_dropped_out_of_order_removes_its_own_frame() {
        ...
        let subscriber = tracing_subscriber::registry().with(TimekettleCorrelationLayer::new());
        tracing::subscriber::with_default(subscriber, || {
            let root = tracing::info_span!("timekettle::http_incoming", request_id = "req-order");
            let _root = root.enter();
            let outer = tracing::info_span!("outer").entered();
            let inner = tracing::info_span!("inner").entered();
            assert_eq!(
                current_span_path().as_deref(),
                Some("timekettle::http_incoming>outer>inner")
            );

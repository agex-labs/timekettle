...
//!
//! The diesel connection wrapper (`timekettle-diesel`) observes each result row's
...
//! takes the rows back out (`TimekettleLoadConnection::take_captured_rows`) and
...
//! implementation detail of `timekettle-diesel`; no host installs, scopes, or reads
//! one.
...
impl WireSlot {
    ...
    /// Put one executed statement's captured rows. Called by the `timekettle-diesel`

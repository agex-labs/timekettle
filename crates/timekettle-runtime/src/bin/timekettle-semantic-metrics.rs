...
use timekettle_runtime::{compute_metrics, read_events, ReplayIndex};
use serde_json::json;
...
fn print_help() {
    eprintln!("Usage: timekettle-semantic-metrics --artifact <DIR> [--write <PATH>]");
}
...
fn semantic_metrics_json(artifact_dir: &Path) -> Result<serde_json::Value, String> {
    ...
    Ok(json!({
        "schema_version": "timekettle.semantic.metrics/v1",

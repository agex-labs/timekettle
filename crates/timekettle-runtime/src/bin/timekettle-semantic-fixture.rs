...
use timekettle_context::ContextSnapshot;
use timekettle_runtime::RecordingHook;
...
struct TimekettlePaymentStore {
    inner: Box<dyn PaymentStore + Send + Sync>,
    hook: Arc<RecordingHook>,
}

delegate_payment_store!(TimekettlePaymentStore, inner, hook, "storage");
...
delegate_config_store!(TimekettleConfigStore, inner, hook, "storage", {
    type Error = String;
});
...
fn main() -> std::io::Result<()> {
    ...
    let mut artifact_dir = env::var_os("TIMEKETTLE_ARTIFACT_DIR").map(PathBuf::from);
    ...
    let state = if mode == "record" {
        let dir =
            artifact_dir.unwrap_or_else(|| PathBuf::from("/tmp/timekettle-semantic-harness/recording"));
        match RecordingHook::new(&dir) {
            Ok(hook) => {
                let hook = Arc::new(hook);
                AppState {
                    payment: Arc::new(TimekettlePaymentStore {
                        inner: Box::new(RealPaymentStore),
                        hook: hook.clone(),
                    }),
                    config: Arc::new(TimekettleConfigStore {
            ...
            Err(error) => {
                eprintln!(
                    "timekettle-semantic-fixture failed to initialize recorder at {}: {error}; continuing without recording",
                    dir.display()
                );
                baseline_state()
            }
    ...
    eprintln!("timekettle-semantic-fixture mode={mode} listening on 127.0.0.1:{port}");
...
fn print_help() {
    eprintln!(
        "Usage: timekettle-semantic-fixture [--mode baseline|record] [--port PORT] [--artifact-dir DIR]"
    );
}
...
fn handle_connection(mut stream: TcpStream, state: AppState) -> std::io::Result<()> {
    ...
    match path {
        ...
        "/payment-flow" => {
            ...
            let result = timekettle_context::scope_sync(ContextSnapshot::new(request_id.clone()), || {

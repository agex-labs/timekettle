...
//! `RuntimeHook`/`TIMEKETTLE_MODE=replay`) that substitutes recorded results
...
//!
//! ```text
//! Handler → TimekettleStore (decorator) → Real Store
//!              │
//!              └─ TimekettleHook::record(BoundaryEvent)
//!                    │
...
//! - `correlation_id`: from `timekettle_context::current_correlation_id()`
...
pub mod writer;
pub use correlation_layer::{current_span_path, TimekettleCorrelationLayer};
pub use graph::{
...
pub const TIMEKETTLE_RUN_ID_ENV_VAR: &str = "TIMEKETTLE_RUN_ID";

pub(crate) fn current_recording_run_id() -> Option<String> {
    std::env::var(TIMEKETTLE_RUN_ID_ENV_VAR)
        .ok()
...
pub struct BoundaryEvent {
    ...
    /// Correlation ID from `timekettle_context::current_correlation_id()`.
...
pub enum TimekettleRecord {
    /// A semantic boundary crossing. Boxed: it is by far the largest variant
    /// (~1 KiB), so keeping it inline would bloat every queued `TimekettleRecord` in
    ...
    /// variant, and an unboxed largest variant sizes every queued `TimekettleRecord`
    /// in the writer channel to it.
    GraphNode(Box<timekettle_core::ExecutionGraphNode>),
...
/// autoref-specialized `timekettle::capture!` (structured serde preferred, tagged
/// `{"debug": …}` fallback, tagged
/// `{"timekettle_unserializable"|"timekettle_opaque_type": …}` markers instead of silent
...
/// Opaque handle returned by [`TimekettleHook::execute_shadow_peek`] and consumed by
/// [`TimekettleHook::execute_shadow_observe`].
///
...
pub struct ExecuteShadowToken {
    ...
    /// is `None` here and filled by [`TimekettleHook::execute_shadow_observe`].
...
/// [`TimekettleHook::try_replay_with_context`].
...
pub trait TimekettleHook: Send + Sync {
    ...
    /// created one instruction before `TimekettleCorrelationLayer::on_enter` engages
    ...
    ///
    /// Default delegation falls back to [`TimekettleHook::try_replay`] using the
...
/// A `TimekettleHook` that does nothing.
///
...
impl TimekettleHook for DisabledHook {
...
pub struct RecordingHook {
    writer: AsyncRecordWriter<TimekettleRecord>,
...
impl RecordingHook {
    ...
    fn verdict_from(&self, decision: Option<timekettle_context::RecordDecision>) -> CaptureVerdict {
        if !self.writer.is_active() {
            return CaptureVerdict::SkipSinkDown;
        }
        match decision {
            Some(timekettle_context::RecordDecision::Record) => CaptureVerdict::Capture,
            Some(timekettle_context::RecordDecision::Skip) => CaptureVerdict::SkipSampledOut,
            None => CaptureVerdict::SkipNoDecision,
        }
    }

    /// Resolve `recording_run_id` from the environment, falling back to a
    /// time-based id when neither `TIMEKETTLE_RECORDING_RUN_ID` nor
    /// `TIMEKETTLE_RUN_ID_ENV_VAR` is set.
    fn resolve_recording_run_id() -> String {
        std::env::var("TIMEKETTLE_RECORDING_RUN_ID")
            .ok()
            .filter(|value| !value.is_empty())
            .or_else(|| {
                std::env::var(TIMEKETTLE_RUN_ID_ENV_VAR)
    ...
    /// sink policy). The sink receives [`TimekettleRecord`]s: boundary events and —
        ...
        S: RecordSink<TimekettleRecord>,
    {
        Self {
            ...
            writer: AsyncRecordWriter::with_seq_of(
                sink,
                writer_config,
                Some(std::sync::Arc::new(TimekettleRecord::global_sequence)),
            ),
...
impl crate::graph::GraphNodeSink for RecordingHook {
    ...
    fn graph_node(&self, mut node: timekettle_core::ExecutionGraphNode) {
        ...
        let _ = self.writer.record(TimekettleRecord::GraphNode(Box::new(node)));
    }
}

impl TimekettleHook for RecordingHook {
    ...
    /// [`capture_verdict`](TimekettleHook::capture_verdict), NOT here — an ingress
    ...
    fn capture_verdict(&self) -> CaptureVerdict {
        ...
        self.verdict_from(timekettle_context::recording_decision_for_current())
    }
    ...
    fn capture_verdict_for(&self, correlation_id: &str) -> CaptureVerdict {
        self.verdict_from(timekettle_context::recording_decision(correlation_id))
    }

    fn record(&self, event: BoundaryEvent) {
        let _ = self
            .writer
            .record(TimekettleRecord::BoundaryEvent(Box::new(event)));
    }
...
impl RuntimeHook {
    ...
    /// The process-level configured mode — see [`TimekettleHook::process_mode`].
    ...
    pub fn process_mode(&self) -> RuntimeMode {
        match self {
            RuntimeHook::Recording(h) => TimekettleHook::process_mode(h.as_ref()),
            RuntimeHook::Replay(h) => TimekettleHook::process_mode(h),
            RuntimeHook::LookupReplay(h) => TimekettleHook::process_mode(h),
            RuntimeHook::Disabled(h) => TimekettleHook::process_mode(h),
        }
    }

    /// The per-boundary capture verdict for this hook — see
    /// [`TimekettleHook::capture_verdict`].
    pub fn capture_verdict(&self) -> CaptureVerdict {
        match self {
            RuntimeHook::Recording(h) => TimekettleHook::capture_verdict(h.as_ref()),
            RuntimeHook::Replay(h) => TimekettleHook::capture_verdict(h),
            RuntimeHook::LookupReplay(h) => TimekettleHook::capture_verdict(h),
            RuntimeHook::Disabled(h) => TimekettleHook::capture_verdict(h),
        }
    }

    /// The capture verdict for a NAMED correlation — see
    /// [`TimekettleHook::capture_verdict_for`].
    ///
    /// This arm-by-arm delegation is not optional. `TimekettleHook::capture_verdict_for`
    ...
    pub fn capture_verdict_for(&self, correlation_id: &str) -> CaptureVerdict {
        match self {
            RuntimeHook::Recording(h) => TimekettleHook::capture_verdict_for(h.as_ref(), correlation_id),
            RuntimeHook::Replay(h) => TimekettleHook::capture_verdict_for(h, correlation_id),
            RuntimeHook::LookupReplay(h) => TimekettleHook::capture_verdict_for(h, correlation_id),
            RuntimeHook::Disabled(h) => TimekettleHook::capture_verdict_for(h, correlation_id),
        }
    }
...
impl TimekettleHook for RuntimeHook {
    fn process_mode(&self) -> RuntimeMode {
    ...
    fn recording_run_id(&self) -> Option<&str> {
        match self {
            ...
            RuntimeHook::Replay(h) => TimekettleHook::recording_run_id(h),
            RuntimeHook::LookupReplay(h) => TimekettleHook::recording_run_id(h),
            RuntimeHook::Disabled(h) => TimekettleHook::recording_run_id(h),
        }
    }
}
...
impl EventBuilder {
    ...
    /// `timekettle_context` correlation ID is used.
    ...
    pub fn start_with_receiver_and_correlation_id(
        ...
        let correlation_id = explicit_correlation_id.or_else(timekettle_context::current_correlation_id);
    ...
    pub fn finish_recorded(self, hook: &dyn TimekettleHook, output: RecordedOutput) {
    ...
    pub fn finish(self, hook: &dyn TimekettleHook, result: serde_json::Value, is_error: bool) {
        let EventBuilder {
            global_sequence,
            request_sequence,
            correlation_id,
...
pub struct LazyEventFinalizer {
    ...
    hook: Option<Arc<dyn TimekettleHook>>,
...
/// [`set_global_runtime_hook`] (e.g. Hyperswitch's `timekettle_boot`, which composes a
...
pub fn observation_is_active() -> bool {
    ...
    // recorder does no prep at all. See [`TimekettleHook::observation_active`]. Resolves
...
/// created before `TimekettleCorrelationLayer::on_enter` makes that correlation
...
impl TaskLineage {
    ...
    /// layer when it sees a `timekettle.fork`-marked span — never from a task-local.
...
/// a `timekettle.fork`-marked span, opens a fresh lineage bucket for the child, and
...
/// `tokio::spawn` hands the child neither the span nor the timekettle context, so every
...
///   child routinely outlives that. `TimekettleCorrelationLayer::on_enter` would then
...
/// [`timekettle_context::capture_current`], which resolves the decision by the one
...
///
/// A bare `tokio::spawn(fut.instrument(timekettle::fork_span()))` is therefore NO
...
where
    ...
    T: Send + 'static,
{
    let context = timekettle_context::capture_current();
    // Dropping the JoinHandle is deliberate — fire-and-forget by contract.
    drop(tokio::spawn(timekettle_context::scope_snapshot(
        context,
        async move {
            let _ = future.await;
        }
...
/// layer (`timekettle.fork = true`). Instrument a spawned future with it —
/// `tokio::spawn(fut.instrument(timekettle::fork_span()))` — to open a fresh lineage
/// bucket for that task.
#[must_use]
pub fn fork_span() -> tracing::Span {
    tracing::info_span!("timekettle.fork", timekettle.fork = true)
}
...
/// [`TimekettleHook::next_callsite_occurrence`] across record / replay / lookup
...
/// The correlation id is read from the ambient `timekettle_context` inside the hook,
...
/// [`TimekettleHook::execute_shadow_peek`]; returns `None` when no hook is configured,
...
/// [`TimekettleHook::execute_shadow_observe`]. No-op when no hook is configured.
///
/// Deprecated: the execute/shadow lifecycle now lives inside [`dispatch`].
#[deprecated(
    since = "0.1.0",
...
pub struct CrossingObservation {
    ...
    /// `timekettle_context` correlation inside the record seam — the same fallback the
...
/// Serving the stale value would lie to every downstream boundary (timekettle
...
/// The stable prefix EVERY timekettle replay fail-stop panic message carries.
///
...
pub const FAIL_STOP_SENTINEL: &str = "timekettle replay fail-stop:";
...
/// Classify a caught panic payload: a timekettle fail-stop, or somebody else's panic.
///
...
/// a timekettle verdict — the same laundering the fail-stop model exists to prevent.
...
///   installed at all, so record-mode and timekettle-off builds keep byte-identical
...
/// when timekettle is active but not recording), which turns `Err(FailStop)` into a
    ...
    Fut: std::future::Future,
{
    ...
    // no behaviour change for a recording or timekettle-off process.
    if !replay_is_active() {
...
fn shadow_observe_loud<F: FnOnce()>(boundary: &str, method: &str, observe: F) {
    if let Err(payload) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(observe)) {
        eprintln!(
            "timekettle: PANIC in execute-shadow observer at `{boundary}::{method}` — \
...
/// explicit `&dyn TimekettleHook` plus the full [`BoundarySpec`] that describes both
...
pub fn read_events(artifact_dir: &Path) -> std::io::Result<Vec<BoundaryEvent>> {
    ...
    for line in content.lines() {
        ...
        // The stream is tagged TimekettleRecords; other kinds (graph nodes) and
        // non-record lines (sink markers) are someone else's to read.
        if let Ok(TimekettleRecord::BoundaryEvent(event)) = serde_json::from_str::<TimekettleRecord>(line) {
            events.push(*event);
        }
    }
    Ok(events)
...
/// implement `TimekettleHook` because returning typed application values from JSON is
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // TimekettleRecord — the one-stream wire shape (tag routes, fields stay flat).
    // -----------------------------------------------------------------------

    #[test]
    fn timekettle_record_wire_shape_is_flat_and_tagged() {
        let node = timekettle_core::ExecutionGraphNode {
            node_id: 7,
            global_sequence: 42,
            parent_id: Some(3),
        ...
        let json = serde_json::to_value(TimekettleRecord::GraphNode(Box::new(node.clone()))).unwrap();
        ...
        let back: TimekettleRecord = serde_json::from_value(json).unwrap();
        assert_eq!(back.global_sequence(), 42);
        match back {
            TimekettleRecord::GraphNode(n) => assert_eq!(*n, node),
            other => panic!("expected GraphNode, got {other:?}"),
        }
    }

    #[test]
    fn timekettle_record_boundary_event_round_trips_with_tag() {
        ...
        let json = serde_json::to_value(TimekettleRecord::BoundaryEvent(Box::new(event))).unwrap();
    ...
    fn recording_hook_mode_obeys_scoped_false_decision() {
        ...
        timekettle_context::set_recording_decision(correlation_id, false);
        {
            let _guard = timekettle_context::enter_correlation_id(correlation_id);
            assert_eq!(hook.capture_verdict(), CaptureVerdict::SkipSampledOut);
        }
        timekettle_context::clear_recording_decision(correlation_id);
    }
    ...
    // fake `TimekettleHook`. A separate test covers the global `dispatch` inactive
    ...
    impl TimekettleHook for FakeHook {
        fn process_mode(&self) -> RuntimeMode {
            if !self.active {
                RuntimeMode::Disabled
    ...
    fn dispatch_with_hook_lookup_hit_result_ok_error_sentinel_fail_stops_before_run() {
        let mut hook = FakeHook::new(true);
        hook.replay_value = Some(serde_json::json!({"timekettle_err": "boom"}));
        let ran = std::cell::Cell::new(false);
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            dispatch_with_hook(
                ...
                |v| {
                    if v.as_object()
                        .is_some_and(|map| map.contains_key("timekettle_err"))
                    {
    ...
    fn dispatch_with_hook_record_and_execute_shadow_use_same_result_extractor() {
        ...
        fn capture_redis_like(value: &Result<RedisLikeValue, ()>) -> (serde_json::Value, bool) {
            match value {
                ...
                Err(()) => (serde_json::json!({"timekettle_err": "()"}), true),
            }
        }
    ...
    fn dispatch_global_inactive_runs_without_evaluating_args_thunk() {
        // No TIMEKETTLE_MODE / TIMEKETTLE_ARTIFACT_DIR set in this unit-test binary, so both

...
//!
//! Provides `ReplayHook` — a `TimekettleHook` that substitutes recorded responses
...
use crate::{
    ...
    CallsiteSource, TimekettleHook, EffectKind, ExecuteMode, OperationKind, ReplayLookup, ReplayStrategy,
    ReturnSemantics, RuntimeMode,
};

const STATE_KEY_V1_PREFIX: &str = "timekettle:state:v1";
...
/// `timekettle-diesel` wrapper when recording, the orchestrator's catalog read when
...
/// A `TimekettleHook` that replays recorded semantic events.
///
...
impl ReplayHook {
    ...
    ///
    /// Used by the env-driven runtime hook (`TIMEKETTLE_MODE=replay`) where no
    ...
    fn lookup_by_identity(
        ...
        let correlation_id = timekettle_context::current_correlation_id();
...
impl TimekettleHook for ReplayHook {
...
//
// Trait surface is dependency-inversion: timekettle-record ships local-file
...
//
// The renderer lives in `timekettle-orchestrator` and the hook is compiled into the
...
// here, in `timekettle-record`, and both sides call it.
// ---------------------------------------------------------------------------
...
/// concrete hook.
pub fn boundary_execute_mode_for(hook: &dyn TimekettleHook, spec: &BoundarySpec) -> ExecuteMode {
...
/// every `#[timekettle::*]` call. Batching, flushing, or sending across the
...
pub trait ObservedCallSink: Send + Sync {
    ...
    fn graph_node(&self, _node: timekettle_core::ExecutionGraphNode) {}
...
pub struct InMemoryObservedSink {
    ...
    graph_nodes: std::sync::Arc<Mutex<Vec<timekettle_core::ExecutionGraphNode>>>,
}
...
impl InMemoryObservedSink {
    ...
    pub fn graph_nodes(&self) -> Vec<timekettle_core::ExecutionGraphNode> {
        self.graph_nodes
            .lock()
            .map(|buf| buf.clone())
            .unwrap_or_default()
...
impl ObservedCallSink for InMemoryObservedSink {
    ...
    fn graph_node(&self, node: timekettle_core::ExecutionGraphNode) {
...
impl FileObservedSink {
    fn write_record(&self, record: &crate::TimekettleRecord) {
        use std::io::Write;
...
impl ObservedCallSink for FileObservedSink {
    fn observed(&self, call: ObservedCall) {
        self.write_record(&crate::TimekettleRecord::Observed(Box::new(call)));
    }
    fn graph_node(&self, node: timekettle_core::ExecutionGraphNode) {
        self.write_record(&crate::TimekettleRecord::GraphNode(Box::new(node)));
    }
    fn flush(&self) -> std::io::Result<()> {
        use std::io::Write;
        if let Ok(mut guard) = self.file.lock() {
...
impl LookupTableHook {
    ...
    pub fn record_graph_node(&self, mut node: timekettle_core::ExecutionGraphNode) {
    ...
    fn resolve(&self, query: &ReplayLookup<'_>) -> Resolution {
        ...
        // ReplayLookup; pull it from the ambient timekettle-context scope set up
...
impl TimekettleHook for LookupTableHook {
    fn process_mode(&self) -> RuntimeMode {
        RuntimeMode::Replay
    }
...
// is a separate, thin wiring step (see `timekettle-orchestrator` lifecycle) that walks
// a `SeedPlan`.
//
...
/// Redis READ events also use the current serde encoding of `TimekettleRedisValue::Null`
...
/// the DB seeder parses its (identically shaped) `TimekettleDatabaseResult` envelope
/// itself.
...
fn image_row_state_key(payload: &serde_json::Value) -> Option<String> {
    ...
    if object.get("timekettle_image").and_then(serde_json::Value::as_str) != Some("db_row") {
        return None;
    }
...
mod tests {
    ...
    /// Mirror the renderer (`timekettle-orchestrator`): walk recorded events and
    ...
    fn boundary_path_event_resolves_at_rank_three() {
        ...
        let _guard = timekettle_context::enter_correlation_id("corr-1");
        let result = hook.try_replay_with_context(ReplayLookup {
            boundary: "redis",
            trait_name: "RedisStore",
            method_name: "get_key",
    ...
    fn span_path_disambiguates_concurrent_same_callsite_calls() {
        ...
        let _guard = timekettle_context::enter_correlation_id("c1");
    ...
    fn callsite_occurrence_is_single_bump_sequence() {
        ...
        let occ = || {
            TimekettleHook::next_callsite_occurrence(
                &hook,
                Some("corr-1"),
                CallsiteSource::SyntacticHash,
    ...
    /// calling `TimekettleHook::next_callsite_occurrence` on the SAME hook that does
    ...
    fn repeated_callsite_resolves_when_occurrence_is_rederived_on_replay() {
        ...
        for i in 0..3u64 {
            let occurrence = TimekettleHook::next_callsite_occurrence(
                &recorder,
                correlation,
                CallsiteSource::SyntacticHash,
                Some(scope),
    ...
    fn seed_plan_skips_declared_redis_raw_null_without_op_metadata() {
        ...
        assert!(
            ...
            "current Redis raw null (`TimekettleRedisValue::Null` => \"Null\") must materialize as absence, not a literal string"
        );
        assert_eq!(
...
";
    ...
    fn hook_execute_shadow_emits_observation_with_real_result() {
        ...
        TimekettleHook::execute_shadow_observe(&hook, token, serde_json::json!(3));
    ...
    fn novel_execute_call_surfaces_as_novel_not_swallowed_seed_gap() {
        ...
        let token = hook.execute_shadow_peek(query).expect("peek token");
        TimekettleHook::execute_shadow_observe(&hook, token, serde_json::json!({"id": "fresh"}));

...
fn install_process_record_hook() {
    ...
    HOOK_DIR.get_or_init(|| {
        ...
        let hook = timekettle_runtime::RecordingHook::new(dir.path()).expect("recording hook");
        timekettle_runtime::set_global_runtime_hook(Some(timekettle_runtime::RuntimeHook::Recording(
            Arc::new(hook),
        )))
...
fn with_decision<T>(decision: bool, f: impl FnOnce() -> T) -> T {
    ...
    let snapshot =
        timekettle_context::ContextSnapshot::new(correlation_id).with_recording_decision(decision);
    let _guard = timekettle_context::enter(snapshot);
    f()
}
...
fn graph_nodes_ride_the_recording_tape() {
    ...
    let hook = Arc::new(timekettle_runtime::RecordingHook::new(dir.path()).expect("recording hook"));
...
// `TimekettleCorrelationLayer`'s span extension yields `None` (that layer runs after
// this one), and reading the ambient `timekettle_context` correlation attributes a
...
//      `TimekettleCorrelationLayer` make the correlation ambient
//
...
/// `.with(timekettle_layer()).with(timekettle_correlation_layer())`, so the graph layer
...
fn collect_graph_at_ingress_order<T>(f: impl Fn() -> T) -> Vec<ExecutionGraphNode> {
    ...
    tracing::subscriber::with_default(
        tracing_subscriber::registry()
            ...
            .with(timekettle_runtime::TimekettleCorrelationLayer::new()),
        || {
            f();
        },
    );
...
fn drive_one_ingress_request(correlation_id: &str) {
    let root = span!(Level::INFO, "timekettle::http_incoming", request_id = %correlation_id);
    let _entered = root.enter();
    // `TracingLogger`'s root span sits inside the timekettle span and names the same
...
fn ingress_root_span_gets_a_node_though_its_correlation_is_not_yet_ambient() {
    ...
    timekettle_context::set_recording_decision(correlation_id, true);
    ...
    let root = node_named(&nodes, "timekettle::http_incoming");
    assert_eq!(
...
fn a_span_naming_an_undecided_correlation_allocates_nothing() {
    ...
    // refuses — note the polarity differs from `TimekettleCorrelationLayer`, which

...
//! Integration test: verify that `#[timekettle::recordable]` + `#[async_trait]`
//! generates a working delegation macro that compiles and records events.

use std::sync::Arc;

use timekettle_runtime::{read_events, RecordingHook};
use serde::Serialize;
...
fn recording_enabled() -> timekettle_context::ContextGuard {
    timekettle_context::enter(
        timekettle_context::ContextSnapshot::new("req-recordable-integration")
            .with_recording_decision(true),
    )
}

// --- Define a trait using #[timekettle::recordable] ---

#[timekettle_derive::recordable]
...
// --- TimekettleStore wrapper using the generated delegation macro ---

struct TimekettleStore {
...
delegate_address_interface!(TimekettleStore, inner, hook, "storage");
...
struct TimekettleConfigStore {
    inner: Box<dyn ConfigInterface<Error = String> + Send + Sync>,
    hook: Arc<RecordingHook>,
}

delegate_config_interface!(TimekettleConfigStore, inner, hook, "storage", {
    type Error = String;
});
...
struct TimekettleSerializeOnlyStore {
    inner: Box<dyn SerializeOnlyInterface + Send + Sync>,
    hook: Arc<RecordingHook>,
}

delegate_serialize_only_interface!(TimekettleSerializeOnlyStore, inner, hook, "storage");
...
struct TimekettleSyncLookupStore {
    inner: RealSyncLookupStore,
    hook: Arc<RecordingHook>,
}

delegate_sync_lookup_interface!(TimekettleSyncLookupStore, inner, hook, "storage");
...
async fn delegation_records_successful_call() {
    ...
    assert!(receiver["self_type"]
        .as_str()
        .is_some_and(|name| name.contains("TimekettleStore")));
...
async fn delegation_with_associated_type_works() {
    ...
    let store = TimekettleConfigStore {
...
async fn recording_only_does_not_require_deserialize_owned_return() {
    ...
    let store = TimekettleSerializeOnlyStore {
...
fn sync_method_with_where_clause_records() {
    ...
    let store = TimekettleSyncLookupStore {
        inner: RealSyncLookupStore,
        hook: hook.clone(),
    };
...
async fn fast_path_skips_recording_when_inactive() {
    use timekettle_runtime::DisabledHook;

    struct TimekettleNoopStore {
        inner: Box<dyn AddressInterface + Send + Sync>,
        hook: Arc<DisabledHook>,
    }

    delegate_address_interface!(TimekettleNoopStore, inner, hook, "storage");

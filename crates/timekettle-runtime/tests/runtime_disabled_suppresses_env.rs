...
fn typed_disabled_hook_exposes_no_recorder_and_keeps_observation_inactive() {
    timekettle_runtime::set_global_runtime_hook(Some(timekettle_runtime::RuntimeHook::Disabled(
        timekettle_runtime::DisabledHook,
    )))
    .expect("install disabled runtime hook");

    assert!(
        timekettle_runtime::global_hook_from_env().is_none(),
        "a typed Disabled runtime hook exposes no recording hook"
    );
    assert!(
        !timekettle_runtime::observation_is_active(),

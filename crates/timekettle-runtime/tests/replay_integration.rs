...
fn recording_enabled() -> timekettle_context::ContextGuard {
    timekettle_context::enter(
        timekettle_context::ContextSnapshot::new("req-replay-integration").with_recording_decision(true),
    )
}
...
struct TimekettleCounter {
    inner: Box<dyn CounterService + Send + Sync>,
    hook: Arc<dyn timekettle_runtime::TimekettleHook>,
}

delegate_counter_service_with_replay!(TimekettleCounter, inner, hook, "service");

struct TimekettleCounterExecute {
    inner: Box<dyn CounterService + Send + Sync>,
    hook: Arc<dyn timekettle_runtime::TimekettleHook>,
}

delegate_counter_service_with_replay!(TimekettleCounterExecute, inner, hook, "service", replay = Execute);
...
async fn replay_returns_recorded_value_without_calling_real_impl() {
    ...
    let replay_store = TimekettleCounter {
...
async fn replay_sliding_window_recovers_skipped_calls() {
    ...
    assert_eq!(
        ...
        timekettle_runtime::DivergenceKind::OmittedCall
    );
}
...
async fn replay_logs_novel_call_and_fail_stops_before_real_impl() {
    ...
    assert_eq!(
        report.divergences[0].kind,
        timekettle_runtime::DivergenceKind::NovelCall
    );
}
...
async fn replay_lookup_hit_with_unreconstructable_result_fail_stops_before_real_impl() {
    ...
    let hook: Arc<dyn timekettle_runtime::TimekettleHook> = Arc::new(
...
async fn replay_arg_mismatch_returns_recorded_result_anyway() {
    ...
    assert_eq!(
        report.divergences[0].kind,
        timekettle_runtime::DivergenceKind::ArgsDiverged
    );
}
...
async fn replay_execute_delegate_runs_real_impl_and_emits_shadow_observation() {
    ...
    let real = RealCounter::new();
    let replay_store = TimekettleCounterExecute {
        inner: Box::new(real.clone()),
        hook,
    };

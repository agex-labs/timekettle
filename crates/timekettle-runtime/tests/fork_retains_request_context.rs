...
use timekettle_runtime::TimekettleCorrelationLayer;
...
fn run_request_forking_a_tail(correlation_id: &str, decision: bool) -> TailSaw {
    ...
    tracing::subscriber::with_default(subscriber, || {
        runtime.block_on(async {
            ...
            timekettle_context::set_recording_decision(correlation_id, decision);

            {
                let request =
                    tracing::info_span!("timekettle::http_incoming", request_id = %correlation_id);
                let _entered = request.enter();

                let saw = Arc::clone(&saw);
                timekettle_runtime::spawn_fork(async move {
                    // Runs after teardown — see the yields below.
                    let hook = timekettle_runtime::installed_runtime_hook().expect("hook installed");
                    *saw.lock().expect("saw lock") = TailSaw {
                        correlation_id: timekettle_context::current_correlation_id(),
            ...
            assert_eq!(
                timekettle_context::recording_decision(correlation_id),
                None,
...
fn a_detached_tail_of_a_sampled_out_request_still_does_not_capture() {
    ...
    // all: `TimekettleCorrelationLayer::on_enter` engages only when the span's

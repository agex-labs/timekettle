...
/// `timekettle::http_incoming` root beside the request tree, holding one event on the
/// record side that replay structurally cannot have.
fn ingress_shaped_nodes() -> Vec<ExecutionGraphNode> {
    vec![
        node(1, 1, None, Some("request-a"), "timekettle::http_incoming"),
...
fn a_zero_event_ingress_root_is_absent_from_the_alignment_on_both_sides() {
    ...
    let alignment = timekettle_forest::align(record_forest, replay_forest);

    assert!(
        !alignment
            .nodes
            .iter()
            .any(|row| row.span_name == "timekettle::http_incoming"),
    ...
    assert!(
        alignment
            ...
            .any(|row| row.span_name == "HTTP request"
                && matches!(row.outcome, timekettle_forest::NodeOutcome::Matched)),

...
//!     cargo test -p timekettle-diesel --test no_eviction -- --ignored --nocapture
//! ```

use timekettle_diesel::{get_result_captured, TimekettleLoadConnection};
...
fn label_bytes(rows: &[timekettle_runtime::wire_capture::WireRow]) -> Option<Vec<u8>> {
    rows.first()?
        .columns
        .iter()

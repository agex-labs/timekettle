...
//!
//! ```text
//! TIMEKETTLE_DIESEL_TEST_DATABASE_URL=postgres://postgres:probe@127.0.0.1:5599/probe \
//!     cargo test -p timekettle-diesel --test pool_handoff_probe -- --ignored --nocapture
//! ```

use timekettle_diesel::{get_result_captured, get_results_captured, TimekettleLoadConnection};
use timekettle_runtime::{RecordingHook, RuntimeHook};
use diesel::pg::PgConnection;
use diesel::sql_types::{BigInt, Text};
use diesel::QueryableByName;
use std::sync::Arc;

type TimekettlePgConnection = TimekettleLoadConnection<PgConnection>;
type TimekettlePool = bb8::Pool<async_bb8_diesel::ConnectionManager<TimekettlePgConnection>>;
...
fn open_gate() {
    ...
    let _ = timekettle_runtime::set_global_runtime_hook(Some(RuntimeHook::Recording(Arc::new(hook))));
...
async fn pool(url: String, size: u32) -> TimekettlePool {
    let manager = async_bb8_diesel::ConnectionManager::<TimekettlePgConnection>::new(url);
    bb8::Pool::builder()
        .max_size(size)
        .build(manager)
        .await
...
fn wire_bytes<'a>(
    rows: &'a [timekettle_runtime::wire_capture::WireRow],
    column: &str,
) -> Option<&'a [u8]> {
    rows.first()?
        .columns
...
#[ignore = "needs a reachable postgres; set TIMEKETTLE_DIESEL_TEST_DATABASE_URL"]
async fn the_pair_survives_the_pool_and_the_blocking_thread() {
    let url = std::env::var("TIMEKETTLE_DIESEL_TEST_DATABASE_URL")
        .expect("TIMEKETTLE_DIESEL_TEST_DATABASE_URL must point at a scratch database");
    open_gate();

    // The production stack: bb8 pool of async-bb8-diesel connections whose
    // inner diesel connection is the timekettle wrapper. Nothing is installed at

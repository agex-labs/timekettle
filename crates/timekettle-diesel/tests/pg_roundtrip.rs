//! Real-postgres round trip for `TimekettleLoadConnection<PgConnection>`: proves the
...
//!
//! ```text
//! TIMEKETTLE_DIESEL_TEST_DATABASE_URL=postgres://user:pass@localhost/db \
//!     cargo test -p timekettle-diesel -- --ignored
//! ```

use timekettle_diesel::TimekettleLoadConnection;
...
fn wrapper_captures_binary_wire_values_from_a_real_result() {
    ...
    let mut conn =
        TimekettleLoadConnection::<PgConnection>::establish(&url).expect("establish through wrapper");
...
fn the_wrapper_satisfies_the_pg_metadata_lookup_bound() {
    ...
    assert_lookup::<TimekettleLoadConnection<PgConnection>>();
}

...
//!     cargo test -p timekettle-diesel --test typed_query_key_probe -- --ignored --nocapture
//! ```

use timekettle_diesel::{first_captured, get_result_captured, TimekettleLoadConnection};
...
async fn a_typed_query_comes_back_paired_with_its_capture() {
    let url =
        std::env::var("TIMEKETTLE_DIESEL_TEST_DATABASE_URL").expect("TIMEKETTLE_DIESEL_TEST_DATABASE_URL");
    open_gate();

    {
        let mut setup =
            TimekettleLoadConnection::<PgConnection>::establish(&url).expect("establish for setup");
    ...
    let conn = async_bb8_diesel::Connection::new(
        TimekettleLoadConnection::<PgConnection>::establish(&url).expect("establish"),
    );

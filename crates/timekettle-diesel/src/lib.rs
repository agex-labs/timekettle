//! `TimekettleLoadConnection<C>` — a diesel connection wrapper that captures each
...
//! into the slot every `TimekettleLoadConnection` owns. The row is handed onward
//! untouched — a pure observer.
//!
//! # The handoff is IN-BAND
//!
//! The capture leaves the connection through [`take_captured_rows`]
//! (`TimekettleLoadConnection::take_captured_rows`), called in the SAME closure that
...
//! `#[timekettle::boundary]`, whose result producer attaches the physical image
//! (`timekettle::db::recorded_output_with_wire`). Nothing ambient — no registry, no
...
//! production; `timekettle_runtime::wire_capture` records the history.)
//!
...
//! pub type PgPool =
//!     bb8::Pool<async_bb8_diesel::ConnectionManager<TimekettleLoadConnection<PgConnection>>>;
//! pub type PgPooledConn =
//!     async_bb8_diesel::Connection<TimekettleLoadConnection<PgConnection>>;
//! ```
//!
...
//!
//! Capture is gated on the process-level timekettle runtime mode, resolved through
//! the SAME installed hook the boundary macros use
//! ([`timekettle_runtime::runtime_mode`]). The gate is process-level rather than
...
use timekettle_runtime::wire_capture::{WireColumn, WireRow, WireSlot};

/// One row of `timekettle::TABLE_IDENTITY_SQL`: a table and one of its primary-key
...
pub struct TimekettleLoadConnection<C> {
    inner: C,
...
impl<C> TimekettleLoadConnection<C> {
...
impl<C> GetPgMetadataCache for TimekettleLoadConnection<C>
where
...
impl<C> SimpleConnection for TimekettleLoadConnection<C>
where
    C: SimpleConnection,
{
...
impl<C> ConnectionSealed for TimekettleLoadConnection<C> {}

impl<C> Connection for TimekettleLoadConnection<C>
where
...
impl<C, B> LoadConnection<B> for TimekettleLoadConnection<C>
where
    C: LoadConnection<B> + Connection<Backend = Pg, TransactionManager = AnsiTransactionManager>,
{
    type Cursor<'conn, 'query>
        = TimekettleCursor<C::Cursor<'conn, 'query>, C::Row<'conn, 'query>>
    where
        Self: 'conn;
    type Row<'conn, 'query>
        = C::Row<'conn, 'query>
        ...
        Self::Backend: diesel::expression::QueryMetadata<T::SqlType>,
    {
        ...
        Ok(TimekettleCursor::new(inner, slot, capture_wire_row))
    }
}

impl<C> diesel::r2d2::R2D2Connection for TimekettleLoadConnection<C>
where
...
pub async fn get_result_captured<C, Q, U>(
    conn: &async_bb8_diesel::Connection<TimekettleLoadConnection<C>>,
    query: Q,
) -> (QueryResult<U>, Option<Vec<WireRow>>)
where
    C: diesel::r2d2::R2D2Connection
        + Connection<Backend = Pg, TransactionManager = AnsiTransactionManager>
        + 'static,
    Q: RunQueryDsl<TimekettleLoadConnection<C>>
        + LoadQuery<'static, TimekettleLoadConnection<C>, U>
        + Send
        + 'static,
    U: Send + 'static,
{
...
where
    C: diesel::r2d2::R2D2Connection
        + Connection<Backend = Pg, TransactionManager = AnsiTransactionManager>
        + 'static,
    Q: RunQueryDsl<TimekettleLoadConnection<C>> + LimitDsl + Send + 'static,
    diesel::dsl::Limit<Q>: LoadQuery<'static, TimekettleLoadConnection<C>, U>,
    U: Send + 'static,
{
    use async_bb8_diesel::AsyncConnection;
    fold_run_outcome(
...
pub struct TimekettleCursor<I, R> {
    inner: I,
    capture: Option<CursorCapture<R>>,
}

impl<I, R> TimekettleCursor<I, R> {
...
impl<I, R> Iterator for TimekettleCursor<I, R>
where
...
impl<I, R> Drop for TimekettleCursor<I, R> {
    fn drop(&mut self) {
...
mod tests {
    ...
    ) -> TimekettleCursor<std::vec::IntoIter<QueryResult<FakeRow>>, FakeRow> {
        TimekettleCursor::new(items.into_iter(), slot, capture_wire_row)
    }

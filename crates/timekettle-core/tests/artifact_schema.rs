...
fn schema_mismatch_rejected() {
    ...
    let mismatched_summary = serde_json::json!({
        "schema_version": "timekettle.artifact/v0",
        "total_records": 4,
        "counts": {
            "time": 1,
            "random": 1,
    ...
    match error {
        ArtifactError::SchemaVersionMismatch {
            ...
            assert_eq!(found, "timekettle.artifact/v0");
        }
        other => panic!("unexpected error: {other:?}"),
    }
    ...
    ArtifactBundle {
        metadata: ArtifactMetadataDocument {
            ...
            preload: Some(PreloadBootstrap::new(
                PreloadMode::Record,
                "/tmp/timekettle-artifact",
                "/tmp/libtimekettle_preload.so",
            )),
        },
        events: vec![
            EventRecord {
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    path.push(format!(
        "timekettle-core-{label}-{}-{timestamp}",
        std::process::id()
    ));
...
fn socket_dns_event_round_trip() {
    ...
    let artifact = ArtifactBundle {
        ...
        events: events.clone(),
        inspection_summary: timekettle_core::inspection_summary_from_events(&events),
    };

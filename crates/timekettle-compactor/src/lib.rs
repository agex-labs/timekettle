//! Compacts timekettle landing objects into the durable session layout (Phase 2.3).
//!
//! Vector lands `timekettle.artifact_record/v2` envelopes as small per-batch
...
impl S3Config {
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            // demo overlay sets TIMEKETTLE_S3_ENDPOINT explicitly for its MinIO.
            endpoint: env("TIMEKETTLE_S3_ENDPOINT", ""),
            bucket: env("TIMEKETTLE_S3_BUCKET", "timekettle-recordings"),
            access_key: env("TIMEKETTLE_S3_ACCESS_KEY", "minioadmin"),
            secret_key: env("TIMEKETTLE_S3_SECRET_KEY", "minioadmin"),
            region: env("TIMEKETTLE_S3_REGION", "us-east-1"),
            // Default true for the demo MinIO (plaintext); a real S3 endpoint
            // sets TIMEKETTLE_S3_ALLOW_HTTP=false to require TLS.
            allow_http: env("TIMEKETTLE_S3_ALLOW_HTTP", "true") != "false",
        }
    }
...
pub struct CodeRef {
    ...
    pub timekettle_version: Option<String>,
}
...
/// `.zst` = the timekettle session layout; `.gz`/gzip-magic = deployed Vector
...
mod tests {
    ...
    fn envelope(inst: &str, gseq: u64, corr: Option<&str>, boundary: &str) -> String {
        ...
        format!(
            r#"{{"schema_version":2,"artifact_type":"timekettle_artifact_record","instance_id":"{inst}","capture":{{"mode":"session","session_id":"s1"}},"code":{{"sha":"abc","timekettle_version":"0.1.0"}},"event":{{"recording_run_id":"s1","global_sequence":{gseq},"correlation_id":{corr_json},"boundary":"{boundary}","event_schema_version":1}}}}"#
        )
    }
    ...
    fn s3_config_requires_both_static_credentials() {
        let mut cfg = S3Config {
            ...
            bucket: "timekettle-recordings".to_owned(),
    ...
    /// (`timekettle-orchestrator/src/s3`), which took it from the producer. It is
    ...
    fn graph_envelope(inst: &str, gseq: u64) -> String {
        format!(
            r#"{{"schema_version":1,"artifact_type":"timekettle_graph_node","instance_id":"{inst}","recording_run_id":"s1","capture":{{"mode":"session","session_id":"s1"}},"node":{{"recording_run_id":"s1","global_sequence":{gseq},"node_id":{gseq},"span_name":"payments_create"}}}}"#
        )
    }
    ...
    fn a_seal_carries_the_graph_nodes_the_recording_captured() {
        ...
        assert_eq!(
            read_back
                .iter()
                .filter(|l| l.contains("timekettle_graph_node"))
                .count(),
            2,
    ...
    fn a_sealed_session_reads_back_the_lines_it_was_sealed_from() {
        ...
        let lines = vec![
            ...
            envelope("i1", 1, Some("c1"), "http_incoming"),
            envelope("i1", 2, Some("c1"), "db"), // duplicate
            r#"{"artifact_type":"timekettle_sink_marker","instance_id":"i1"}"#.to_owned(),
        ];
        let manifest = seal(&store, "s1", &lines);
